...
function getBattleZonePoint(zone){
    const point=ZONE_POSITIONS && ZONE_POSITIONS[zone];
    return point ? {x:point.x,y:point.y} : null;
}

function setBattleBeyVisual(id,labelId,zone,offsetX=0){
    const point=getBattleZonePoint(zone);
    const bey=document.getElementById(id);
    const label=document.getElementById(labelId);
    if(!point || !bey) return;
    const x=point.x+offsetX;
    bey.setAttribute("cx",x);
    bey.setAttribute("cy",point.y);
    if(label){
...
function ensureBattleVisualStyles(){
    if(document.getElementById("battleVisualStyles")) return;
    const style=document.createElement("style");
    style.id="battleVisualStyles";
    style.textContent=`
      .battle-card{max-width:620px!important;padding:12px 14px!important;}
      .battle-header{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:4px;}
      .battle-header h1{margin:0!important;font-size:18px!important;}
      .battle-replay{padding:6px 10px!important;font-size:10px!important;white-space:nowrap;}
      .battle-meta{font-size:9px;opacity:.58;text-align:center;margin-bottom:4px;}
      .battle-commentary{margin-top:6px!important;padding:8px 10px!important;}
      .battle-commentary .battle-event-text{font-size:11px;line-height:1.3;}
      .battle-timeline{display:flex;gap:4px;overflow:hidden;margin:5px 0!important;}
      .battle-timeline button{font-size:8px!important;min-width:62px!important;padding:5px 6px!important;}
      .battle-controls{display:flex;justify-content:space-between;align-items:center;gap:6px;margin:5px 0!important;}
      .battle-controls .menu-btn{padding:6px 9px!important;font-size:9px!important;}
      .battle-impact-label{font-weight:900;letter-spacing:1px;font-size:13px;pointer-events:none;}
      .battle-impact-ring{fill:none;stroke-width:7;opacity:0;pointer-events:none;}
      .battle-impact-core{opacity:0;pointer-events:none;}
      .battle-impact-particle{stroke-width:3;stroke-linecap:round;opacity:0;pointer-events:none;}
      @media(max-width:700px){
        .battle-card{width:calc(100vw - 18px)!important;padding:9px 10px!important;}
        .battle-header h1{font-size:16px!important;}
      }
    `;
    document.head.appendChild(style);
}

function getImpactVisualProfile(event,damage){
    const e=String(event||"");
    let power=40,label="IMPACT";
    if(/extreme/i.test(e)){power=100;label="EXTREME IMPACT";}
    else if(/rail/i.test(e)){power=90;label="X-RAIL HIT";}
    else if(/heavy/i.test(e)){power=80;label="HEAVY HIT";}
    else if(/counter/i.test(e)){power=65;label="COUNTER HIT";}
    else if(/normal|collision|impact/i.test(e)){power=50;label="HIT";}
    else if(/pressure/i.test(e)){power=30;label="PRESSURE";}
    else if(/glanc|nearMiss/i.test(e)){power=20;label="GLANCING HIT";}
    const d=damage?.effectiveImpact || damage?.impactBase || 0;
    power=Math.max(power,Math.min(100,Math.round(Number(d)||0)));
    return {power,label};
}

function clearBattleImpactFx(){
    const old=document.getElementById("battleImpactFx");
    if(old) old.remove();
}

function playBattleImpactFx(x,y,profile,token){
    ...
    ring.classList.add("battle-impact-ring");
    ring.setAttribute("r",profile.power>=85?12:profile.power>=60?10:8);
    ring.setAttribute("stroke",profile.power>=85?"#fff":profile.power>=60?"#ffd43b":"#d8e8ff");
    g.appendChild(ring);

    const core=document.createElementNS(ns,"circle");
    core.classList.add("battle-impact-core");
    core.setAttribute("r",profile.power>=85?15:profile.power>=60?12:9);
    core.setAttribute("fill",profile.power>=85?"#ffffff":"#ffd43b");
    g.appendChild(core);

    const count=profile.power>=90?12:profile.power>=75?9:profile.power>=55?7:5;
    for(let i=0;i<count;i++){
        const line=document.createElementNS(ns,"line");
        line.classList.add("battle-impact-particle");
        const a=(Math.PI*2*i/count)+(Math.random()*0.18);
        line.dataset.angle=a;
        line.dataset.length=String(profile.power>=85?28:profile.power>=60?21:15);
        line.setAttribute("x1",Math.cos(a)*7);
        line.setAttribute("y1",Math.sin(a)*7);
        line.setAttribute("x2",Math.cos(a)*7);
        line.setAttribute("y2",Math.sin(a)*7);
        line.setAttribute("stroke",profile.power>=85?"#fff":profile.power>=60?"#ffd43b":"#d8e8ff");
        g.appendChild(line);
    }
    const text=document.createElementNS(ns,"text");
    text.classList.add("battle-impact-label");
    text.setAttribute("text-anchor","middle");
    text.setAttribute("y","-25");
    text.setAttribute("fill",profile.power>=85?"#fff":"#ffd43b");
    text.textContent=profile.label;
    g.appendChild(text);
    svg.appendChild(g);

    const start=performance.now();
    const duration=profile.power>=80?700:500;
    const frame=now=>{
        if(token!==Game.battle.sequencePlaybackToken){g.remove();return;}
        const t=Math.min(1,(now-start)/duration);
        const pulse=t<0.45 ? t/0.45 : 1-(t-0.45)/0.55;
        ring.setAttribute("r",String((profile.power>=85?12:8)+pulse*(profile.power>=85?44:28)));
        ring.style.opacity=String(Math.max(0,pulse*0.95));
        core.style.opacity=String(Math.max(0,pulse*0.9));
        core.setAttribute("r",String((profile.power>=85?15:9)+pulse*(profile.power>=85?16:10)));
        text.style.opacity=String(Math.max(0,Math.min(1,pulse*1.2)));
        g.querySelectorAll(".battle-impact-particle").forEach(line=>{
            const a=Number(line.dataset.angle), len=Number(line.dataset.length);
            const l=len*pulse;
            line.setAttribute("x1",String(Math.cos(a)*8));
            line.setAttribute("y1",String(Math.sin(a)*8));
            line.setAttribute("x2",String(Math.cos(a)*l));
            line.setAttribute("y2",String(Math.sin(a)*l));
            line.style.opacity=String(Math.max(0,pulse));
        });
...
function animateBattleSequence(sequence,index,replay=false){
    ...
    const movement=a.movement || a;
    const pFrom=getBattleZonePoint(movement.player?.from), pTo=getBattleZonePoint(movement.player?.to);
    const cFrom=getBattleZonePoint(movement.cpu?.from), cTo=getBattleZonePoint(movement.cpu?.to);
    if(!pFrom || !pTo || !cFrom || !cTo) return;

    const event=a.event || sequence.event || "";
    const profile=getImpactVisualProfile(event,a.impact);
    const impactEvent=profile.power>=45 && !!(a.winner || a.impact || /hit|impact|collision|pressure|rail/i.test(event));
    const samePlayer=pFrom.x===pTo.x && pFrom.y===pTo.y;
    const sameCpu=cFrom.x===cTo.x && cFrom.y===cTo.y;
    if(samePlayer && sameCpu && !impactEvent){ renderBeys(); return; }

    const pOffset=movement.player.from===movement.cpu.from ? -12 : 0;
    const cOffset=movement.player.from===movement.cpu.from ? 12 : 0;
    const winner=a.winner || (a.impact?.winner) || null;
    const loser=a.loser || (winner ? (winner==="player"?"cpu":"player") : null);
    const baseDuration=replay?950:780;
    const approachDuration=impactEvent ? (replay?1150:1050) : baseDuration;
    const impactPause=impactEvent ? (profile.power>=80?520:360) : 0;
    const recoilDuration=impactEvent ? (profile.power>=80?760:520) : 0;
    const start=performance.now();

    const contactX=(pTo.x+cTo.x)/2;
    const contactY=(pTo.y+cTo.y)/2;
    const attackerPoint=winner==="cpu" ? cTo : pTo;
    const defenderPoint=loser==="cpu" ? cTo : pTo;

    const setPos=(el,label,x,y)=>{
        el.setAttribute("cx",x); el.setAttribute("cy",y);
        if(label){label.setAttribute("x",x);label.setAttribute("y",y-18);}
    };
    if(playerLabel) playerLabel.textContent="YOU";
    if(cpuLabel) cpuLabel.textContent="CPU";
    setPos(player,playerLabel,pFrom.x+pOffset,pFrom.y);
    setPos(cpu,cpuLabel,cFrom.x+cOffset,cFrom.y);
    ...
    const frame=now=>{
        ...
        if(impactEvent && elapsed<approachDuration){
            const t=Math.min(1,elapsed/approachDuration);
            const e=t<0.5?2*t*t:1-Math.pow(-2*t+2,2)/2;
            // Both Beys visibly converge on the same collision point.
            px=pFrom.x+(contactX-pFrom.x)*e+pOffset;
            py=pFrom.y+(contactY-pFrom.y)*e;
            cx=cFrom.x+(contactX-cFrom.x)*e+cOffset;
            cy=cFrom.y+(contactY-cFrom.y)*e;
        }else if(impactEvent && elapsed<approachDuration+impactPause){
            px=contactX-12; py=contactY;
            cx=contactX+12; cy=contactY;
            if(!impactShown){
                impactShown=true;
                playBattleImpactFx(contactX,contactY,profile,token);
            }
        }else if(impactEvent){
            const t=Math.min(1,(elapsed-approachDuration-impactPause)/recoilDuration);
            const e=t<0.5?2*t*t:1-Math.pow(-2*t+2,2)/2;
            px=(winner==="player"?contactX-12:pTo.x)+(winner==="player"?(pTo.x-(contactX-12))*e:0);
            py=(winner==="player"?contactY:pTo.y);
            cx=(winner==="cpu"?contactX+12:cTo.x)+(winner==="cpu"?(cTo.x-(contactX+12))*e:0);
            cy=(winner==="cpu"?contactY:cTo.y);
            if(winner==="player"){
                cx=(contactX+12)+(cTo.x-(contactX+12))*e;
                cy=contactY+(cTo.y-contactY)*e;
            }else if(winner==="cpu"){
                px=(contactX-12)+(pTo.x-(contactX-12))*e;
                py=contactY+(pTo.y-contactY)*e;
            }else{
                px=pFrom.x+(pTo.x-pFrom.x)*e+pOffset;
                py=pFrom.y+(pTo.y-pFrom.y)*e;
                cx=cFrom.x+(cTo.x-cFrom.x)*e+cOffset;
                cy=cFrom.y+(cTo.y-cFrom.y)*e;
            }
        }else{
        ...
        const total=impactEvent?approachDuration+impactPause+recoilDuration:baseDuration;
        if(elapsed<total){
            requestAnimationFrame(frame);
        }else{
            clearBattleImpactFx();
...
function hookMenuButtons(){
    ...
    const buttons = document.querySelectorAll(".menu-btn[data-mode]");
...
function saveBattleSequence(title,text){
    ...
    Game.battle.history.push({
        ...
        animation:(title!=="TACTICAL DECISION" && Game.battle.lastMovement) ? {
            movement:JSON.parse(JSON.stringify(Game.battle.lastMovement)),
            event:Game.battle.lastEvent || null,
            winner:Game.battle.lastHitWinner || null,
            loser:Game.battle.lastHitWinner ? (Game.battle.lastHitWinner==="player" ? "cpu" : "player") : null,
            impact:Game.battle.lastHitDamage ? JSON.parse(JSON.stringify(Game.battle.lastHitDamage)) : null
        } : null
    });
...
function renderBattleSequence(){
    ...
    const timeline=history.slice(Math.max(0,history.length-7)).map((entry,i)=>{
        ...
        return `<button type="button" data-history-index="${safeIndex}" style="flex:1 1 62px;min-width:62px;padding:5px 6px;border-radius:7px;border:1px solid ${active?'rgba(255,212,59,.65)':'rgba(255,255,255,.08)'};background:${active?'rgba(255,212,59,.12)':'rgba(255,255,255,.04)'};color:inherit;text-align:left;font-size:8px;cursor:pointer;">${safeIndex+1}. ${label}</button>`;
    }).join("");

    let decisionPanel="";
    if(live && Game.battle.decisionActive && Game.battle.pendingDecision){
        const d=Game.battle.pendingDecision;
        decisionPanel=`<div style="margin-top:7px;padding:9px;border-radius:9px;border:1px solid rgba(255,212,59,.45);background:rgba(255,212,59,.06);">
            <div style="font-weight:900;font-size:11px;">TACTICAL DECISION</div>
            <p style="line-height:1.3;font-size:10px;margin:4px 0 7px;">${d.scenario}</p>
            ${d.choices.map((choice,i)=>{
                const profile=getDecisionProfile(choice.intent);
                return `<button type="button" class="menu-btn gold tactical-choice-btn" style="margin-top:4px;padding:7px 9px;font-size:10px;text-align:left;cursor:pointer;position:relative;z-index:20;pointer-events:auto;" data-tactical-intent="${choice.intent}">
                    <div>${i+1}. ${choice.name}</div>
                    <div style="font-size:8px;opacity:.65;margin-top:2px;">RISK ${profile.risk} · REWARD ${profile.reward} · ${profile.desc}</div>
                </button>`;
            }).join("")}
        </div>`;
    }
    ...
    if(isResult){
        ...
        resultPanel=`<div style="margin:7px 0 8px;padding:10px;border-radius:10px;border:1px solid rgba(255,212,59,.5);background:rgba(255,212,59,.08);text-align:center;">
            <div style="font-size:9px;opacity:.7;letter-spacing:.7px;">${sequence.finish==="Xtreme Finish" ? "XTREME FINISH" : "ROUND COMPLETE"}</div>
            <h2 style="margin:3px 0 2px;font-size:18px;">${winnerName} WINS</h2>
            <div style="font-size:11px;font-weight:800;">${sequence.finish || "Spin Finish"} · +${points} POINT${points===1?'':'S'}</div>
            <div style="display:flex;justify-content:center;gap:20px;margin-top:6px;font-size:11px;"><strong>YOU ${displayScore.player}</strong><strong>CPU ${displayScore.cpu}</strong></div>
        </div>`;
    }
    ...
    <div class="background"></div>
    <main class="menu">
      <section class="menu-card battle-card">
        <div class="battle-header">
          <h1>ROUND ${sequence.round || Game.battle.round || 1} · BATTLE</h1>
          <button class="menu-btn silver battle-replay" id="replaySequence" type="button">↻ REPLAY</button>
        </div>
        <div class="battle-meta">STEP ${index+1}/${history.length} · YOU ${displayScore.player} — CPU ${displayScore.cpu}</div>
        ${renderStadium()}
        <div class="battle-commentary">
          <div style="font-size:9px;opacity:.58;letter-spacing:.7px;margin-bottom:3px;">COMMENTARY</div>
          <div class="battle-event-text" style="white-space:pre-line;">${sequence.text}</div>
        </div>
        ${renderBattleStatusPanel()}
        <div class="battle-timeline">${timeline}</div>
        <div class="battle-controls">
          <button class="menu-btn silver" id="previousSequence" ${previousDisabled}>← BACK</button>
          <span style="font-size:8px;opacity:.5;">HISTORY</span>
          <button class="menu-btn silver" id="nextSequence" ${nextDisabled}>NEXT →</button>
        </div>
        ${resultPanel}
        ${decisionPanel}
        <div style="margin-top:6px;">${continueButton}</div>
      </section>
    </main>`;
