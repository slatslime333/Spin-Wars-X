...
function getAnimationPath(animation,side){
    ...
    if(!m) return [];
    const zones=getMovementPath(m.from,m.to);
    const points=zones.map(z=>ZONE_POSITIONS[z]).filter(Boolean);
    if(points.length<2){
...
<g id="trajectoryLayer" pointer-events="none">
    <path id="playerTrajectory" d="" fill="none" stroke="#3ba8ff" stroke-width="3" stroke-dasharray="7 8" opacity="0"/>
    <path id="cpuTrajectory" d="" fill="none" stroke="#ff4b4b" stroke-width="3" stroke-dasharray="7 8" opacity="0"/>
</g>
...
function updateTrajectoryPreview(movement){
    ...
    setPath(playerPath,"player","#3ba8ff");
    setPath(cpuPath,"cpu","#ff4b4b");
}
...
function animateBattleSequence(sequence,index,replay=false){
    ...
    const frame=(now)=>{
        ...
        // Slow motion around the collision: approach normally, then dwell at
        // contact so the player can actually see the hit.
        let visualT=t;
        if(impact){
            if(t<0.70){
                visualT=t/0.70*0.82;
                ...
                visualT=0.82+(t-0.86)/0.14*0.18;
            }
        }

        const pp=pointAt(pPath,visualT);
        const cp=pointAt(cPath,visualT);

        // If both paths end in the same zone, separate the circles slightly
        // until the impact frame so the contact is visible.
        let px=pp.x, py=pp.y, cx=cp.x, cy=cp.y;
        if(impact && t>=0.66 && t<0.90 && pPath[pPath.length-1] && cPath[cPath.length-1]){
            const dx=cx-px, dy=cy-py, len=Math.hypot(dx,dy)||1;
            const sep=t<0.80 ? 18*(1-(t-.66)/.14) : 0;
            px-=dx/len*sep; py-=dy/len*sep;
            cx+=dx/len*sep; cy+=dy/len*sep;
        }
...
function applyBattleEvent(event){
    ...
    const spinDamage=Math.max(
        strength>=1.15 ? 4 : 2,
        Math.round((4.2+cleanAdvantage*0.19)*strength*variance)
    );
    const balanceDamage=Math.max(
        strength>=1.15 ? 2 : 1,
        Math.round((2.1+cleanAdvantage*0.10)*strength*variance)
    );
