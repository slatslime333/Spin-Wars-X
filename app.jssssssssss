...
function animateBattleSequence(sequence,index,replay=false){
    ...
    const duration=(impact ? 1150 : 700) * (replay ? 1.12 : 1);
    ...
    const finishImpact=()=>{
        ...
        flash.setAttribute("r",strength==="HEAVY" ? "36" : strength==="STRONG" ? "30" : "24");
        ring.setAttribute("r","8");
        let step=0;
        const pulse=()=>{
            if(token!==Game.battle.sequencePlaybackToken) return;
            step++;
            const scale=1+step*0.18;
            ring.setAttribute("r",String(8+step*7));
            flash.setAttribute("r",String((strength==="HEAVY"?36:strength==="STRONG"?30:24)*scale));
            g.setAttribute("opacity",String(Math.max(0,1-step/9)));
            if(step<9) requestAnimationFrame(pulse);
        };
    ...
    const frame=(now)=>{
        ...
        const cy=cFrom.y+(cTo.y-cFrom.y)*e+cOffset*0;

        // For impacts, hold the final contact for a brief "slow-mo" beat.
        const contactHold=impact && t>0.78 && t<0.91;
        const holdT=contactHold ? 0.78 : t;
        const pe=holdT<0.5 ? 2*holdT*holdT : 1-Math.pow(-2*holdT+2,2)/2;
        const fx=pFrom.x+(pTo.x-pFrom.x)*pe+pOffset;
        const fy=pFrom.y+(pTo.y-pFrom.y)*pe;
        const gx=cFrom.x+(cTo.x-cFrom.x)*pe+cOffset;
        const gy=cFrom.y+(cTo.y-cFrom.y)*pe;

        setPos(player,playerLabel,fx,fy);
        setPos(cpu,cpuLabel,gx,gy);
