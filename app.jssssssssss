...
function animateBattleSequence(sequence,index,replay=false){
    ...
    const pFrom=getBattleZonePoint(a.player.from), pTo=getBattleZonePoint(a.player.to);
    const cFrom=getBattleZonePoint(a.cpu.from), cTo=getBattleZonePoint(a.cpu.to);
    if(!pFrom || !pTo || !cFrom || !cTo) return;

    const samePlayer=pFrom.x===pTo.x && pFrom.y===pTo.y;
    const sameCpu=cFrom.x===cTo.x && cFrom.y===cTo.y;
    if(samePlayer && sameCpu){
        renderBeys();
        return;
    }

    const pOffset=a.player.from===a.cpu.from ? -12 : 0;
    const cOffset=a.player.from===a.cpu.from ? 12 : 0;
    const duration=replay ? 850 : 700;
    ...
    const frame=(now)=>{
        ...
        const px=pFrom.x+(pTo.x-pFrom.x)*e+pOffset;
        const py=pFrom.y+(pTo.y-pFrom.y)*e;
        const cx=cFrom.x+(cTo.x-cFrom.x)*e+cOffset;
        const cy=cFrom.y+(cTo.y-cFrom.y)*e;

        player.setAttribute("cx",px);
        player.setAttribute("cy",py);
        cpu.setAttribute("cx",cx);
        cpu.setAttribute("cy",cy);
        if(playerLabel){playerLabel.setAttribute("x",px);playerLabel.setAttribute("y",py-18);}
        if(cpuLabel){cpuLabel.setAttribute("x",cx);cpuLabel.setAttribute("y",cy-18);}
    ...
    // Start from the recorded state, not from whatever the live simulation
    // happens to be doing. This makes replay deterministic.
    if(playerLabel) playerLabel.textContent="YOU";
    if(cpuLabel) cpuLabel.textContent="CPU";
    player.setAttribute("cx",pFrom.x+pOffset);
    player.setAttribute("cy",pFrom.y);
    cpu.setAttribute("cx",cFrom.x+cOffset);
    cpu.setAttribute("cy",cFrom.y);
    requestAnimationFrame(frame);
}
...
function renderBattleSequence(){
    ...
    <main class="menu">
      <section class="menu-card" style="max-width:560px;">
        ...
        <hr>
        ${renderStadium()}
        <div style="margin-top:8px;padding:9px 10px;border-radius:10px;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.08);">
          <div style="font-size:9px;opacity:.6;letter-spacing:.7px;margin-bottom:4px;">COMMENTARY</div>
        ...
        <div style="display:flex;justify-content:flex-end;margin-top:6px;">
          <button class="menu-btn silver" id="replaySequence" style="padding:6px 10px;font-size:9px;">↻ REPLAY</button>
        </div>
        ${resultPanel}
        ${decisionPanel}
    ...
    if(replay){
        replay.onclick=()=>animateBattleSequence(sequence,index,true);
    }
    if(sequence.animation){
        animateBattleSequence(sequence,index,false);
    }
