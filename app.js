...
function getSimpleMoveStat(blader,intent){
    ...
    switch(intent){
        ...
        case "chase":
        case "counter":
            return Math.round(((s.attack||60)+(s.knockback||60))/2);
        case "brace":
            return Math.round(((s.defense||60)+(s.balance||60))/2);
        case "evade":
        case "escape":
        case "center":
            return Math.round(s.mobility||60);
        case "stabilize":
        case "hold":
            return Math.round(((s.balance||60)+(s.stamina||60))/2);
        default:
            return 60;
    }
}
...
showIntentResult(
    ...
    // A decision changes the exchange; it does not freeze the battle.
    Game.battle.lastEvent=null;
    Game.battle.decisionCooldown=2;
...
function applyBattleEvent(event){
    ...
    const advantage=Math.max(0,attackScore+momentumBonus-defenseScore);

    let strength=1;
    if(event==="glancingHit") strength=0.6;
    if(event==="pressure") strength=0.8;
    if(event==="normalHit") strength=1;
    if(event==="counterHit") strength=1.25;
    if(event==="heavyHit") strength=1.6;
    if(event==="extremeImpact") strength=2;
    if(event==="railImpact") strength=1.8;

    const spinDamage=Math.max(1,Math.round((2+advantage/6)*strength+Math.random()*2));
    const balanceDamage=Math.max(1,Math.round((1+advantage/8)*strength+Math.random()*2));
...
function getImpactCalculationText(){
    ...
    return `SIMPLE CALC · ${winnerName}
ATTACK SCORE · (ATK ${d.attackPower} + KB ${d.knockbackPower}) ÷ 2 + MOM ${d.momentumBonus >= 0 ? d.momentumBonus : 0}
DEFENSE SCORE · (DEF ${d.defenderDefense} + BAL ${d.defenderBalance}) ÷ 2
RESULT · ${winnerName} vs ${loserName} · advantage ${Math.round(d.impactBase)}
...
function decideNextBattleStep(){
    ...
    // After a decision, always simulate before asking for another one.
