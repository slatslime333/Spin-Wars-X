...
function renderBattleStatus(key){
    ...
    return `
        <div style="padding:14px;border-radius:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.10);">
            ...
            <div style="margin-top:6px;font-size:12px;opacity:.72;">${getStateDetail(key)} · NOW: ${formatBattleZone(b.zone)}</div>
        </div>`;
}
...
function simulateBattleMovement(bey){
    ...
    for(const zone of neighbors){
        ...
        if(approachesOpponent) score+=(aggression*0.25)+(Math.max(0,momentum)*0.35);
        ...
        if(type==="Stamina" && rail) score-=risk*0.8;
        if(type==="Defense" && rail) score-=risk*0.35;
        if(spin<45 && center) score+=20;
...
function resolveAutomaticSituation(){
    ...
    const pPower=(pCombo?.stats.attack||60)*0.5+(pCombo?.stats.knockback||60)*0.4+player.speed*0.15+player.momentum*0.30;
    const cPower=(cCombo?.stats.attack||60)*0.5+(cCombo?.stats.knockback||60)*0.4+cpu.speed*0.15+cpu.momentum*0.30;
    ...
    if(railSide){
        event=roll<55 ? "railDash" : "railImpact";
    }else if(situation==="separated"){
        if(roll<45) event="reposition";
        else if(roll<65) event="nearMiss";
        else event="separation";
    }else if(near){
        if(roll<18) event="nearMiss";
        else if(roll<38) event="glancingHit";
        else if(roll<55) event="pressure";
        else if(roll<78) event="normalHit";
        else event="heavyHit";
    }else if(close){
        const attacker=pPower>=cPower ? "player" : "cpu";
        const attackerState=Game.battle[attacker];
        const defender=attacker==="player" ? "cpu" : "player";
        if(Game[defender].currentMove==="Counter" && roll<38){
...
function applyBattleEvent(event){
    ...
    const spinDamage=Math.max(2,Math.round((4+impactBase*0.10)*strength + Math.random()*3 + Math.max(0,winnerState.momentum)*0.025));
    const balanceDamage=Math.max(2,Math.round((3+impactBase*0.07)*strength + Math.random()*4));
...
${Game.player.blade.name}: ${formatBattleZone(Game.battle.player.zone)}
${Game.cpu.blade.name}: ${formatBattleZone(Game.battle.cpu.zone)}`;
    const movementText=`MOVEMENT THIS STEP
${Game.player.blade.name}: ${playerMove}
${Game.cpu.blade.name}: ${cpuMove}`;

    Game.battle.sequenceEvents.push(`${movementText}

${currentPosition}

${eventText}`);

    const target=Game.battle.sequenceTarget || (3+Math.floor(Math.random()*2));
    ...
    if(!shouldShow){
        setTimeout(()=>decideNextBattleStep(),180);
        return;
    }

    const text=Game.battle.sequenceEvents.join("\n\n━━━━━━━━━━━━━━━━━━\n\n");
...
function startBattleRound(){
    ...
    Game.battle.finished=false;
    Game.battle.maxTurns=Math.floor(Math.random()*3)+5;
    Game.battle.decisionActive=false;
    Game.battle.pendingDecision=null;
...
function renderBattleSequence(){
    ...
    const log=recent.map((entry,i)=>{
        const actualIndex=history.length-recent.length+i;
        const active=actualIndex===index;
        return `<div style="padding:12px 14px;margin-bottom:8px;border-radius:10px;background:${active?'rgba(255,255,255,.11)':'rgba(255,255,255,.045)'};border:1px solid ${active?'rgba(255,212,59,.55)':'rgba(255,255,255,.06)'};">
            <div style="font-weight:800;font-size:12px;letter-spacing:.4px;opacity:.9;">STEP ${actualIndex+1} · ${entry.title}</div>
            <div style="margin-top:7px;white-space:pre-line;line-height:1.5;font-size:12px;">${entry.text}</div>
        </div>`;
    }).join("");

    const p=Game.battle.player, c=Game.battle.cpu;
    const positionSummary=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0;">
        <div style="padding:10px;border-radius:9px;background:rgba(59,168,255,.10);font-size:12px;"><strong>${Game.player.blade.name}</strong><br>NOW: ${formatBattleZone(p.zone)}</div>
        <div style="padding:10px;border-radius:9px;background:rgba(255,75,75,.10);font-size:12px;"><strong>${Game.cpu.blade.name}</strong><br>NOW: ${formatBattleZone(c.zone)}</div>
    </div>`;

    let decisionPanel="";
    if(live && Game.battle.decisionActive && Game.battle.pendingDecision){
        const d=Game.battle.pendingDecision;
        decisionPanel=`<div style="margin-top:12px;padding:14px;border-radius:12px;border:1px solid rgba(255,212,59,.45);background:rgba(255,212,59,.07);">
            <div style="font-weight:900;font-size:14px;">TACTICAL DECISION</div>
            <p style="line-height:1.5;font-size:13px;margin:8px 0 12px;">${d.scenario}</p>
            ${d.choices.map((choice,i)=>`<button class="menu-btn gold decision-btn" style="margin-top:7px;" onclick="chooseDynamicMove('${choice.intent}')">${i+1}. ${choice.name}</button>`).join("")}
        </div>`;
    }

    const continueButton=(!Game.battle.decisionActive && live) ? `<button class="menu-btn gold" id="continueBattle">CONTINUE</button>` : "";

    app.innerHTML=`
    <div class="background"></div>
    <main class="menu">
      <section class="menu-card" style="max-width:560px;">
        <h1>ROUND ${Game.battle.round || 1} · BATTLE</h1>
        <div style="font-size:12px;opacity:.7;margin-top:-5px;">Step ${index+1} / ${history.length} · Map shows CURRENT positions</div>
        <hr>
        ${renderStadium()}
        ${positionSummary}
        ${renderBattleStatusPanel()}
        <div style="display:flex;justify-content:space-between;align-items:center;margin:14px 0;gap:10px;">
          <button class="menu-btn silver" id="previousSequence" ${previousDisabled}>← BACK</button>
          <strong style="font-size:12px;">BATTLE HISTORY</strong>
          <button class="menu-btn silver" id="nextSequence" ${nextDisabled}>NEXT →</button>
        </div>
        <div id="battleHistory" style="max-height:310px;overflow-y:auto;padding-right:4px;">${log}</div>
        ${decisionPanel}
        <div style="margin-top:12px;">${continueButton}</div>
      </section>
    </main>`;
    ...
    const historyBox=document.getElementById("battleHistory");
    if(historyBox && live) historyBox.scrollTop=historyBox.scrollHeight;
}
//=========================
// CONTINUE BATTLE
//=========================
