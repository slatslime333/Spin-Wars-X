...
const BLADE_ENGINE = {
    ...
    silver_wolf:{
        ...
        card:{ovr:93,attack:60,knockback:61,defense:90,mobility:58,balance:91,stamina:94,burst:91},
    ...
    phoenix_wing:{
        ...
        card:{ovr:90,attack:86,knockback:88,defense:80,mobility:86,balance:86,stamina:74,burst:87},
    ...
    shark_edge:{
        ...
        card:{ovr:83,attack:93,knockback:95,defense:61,mobility:90,balance:62,stamina:57,burst:74},
    ...
    knight_mail:{
        ...
        card:{ovr:85,attack:71,knockback:88,defense:85,mobility:60,balance:84,stamina:69,burst:91},
    ...
    shelter_drake:{
        ...
        spin:"Right",
        weight:32.4,
        sprite:"assets/blades/shelterdrake.png?v=8.90",

        card:{ovr:71,attack:68,knockback:69,defense:67,mobility:80,balance:75,stamina:78,burst:70},
    ...
    viper_tail:{
        ...
        card:{ovr:73,attack:75,knockback:76,defense:63,mobility:83,balance:66,stamina:63,burst:70},
...
const RATCHET_SIDE_DELTA={
    ...
    7:{attack:0,knockback:1,defense:2,balance:2,stamina:2,mobility:-1,burst:0},
    9:{attack:-1,knockback:-1,defense:1,balance:2,stamina:2,mobility:-1,burst:5}
};
const BIT_STAT_DELTA={
    Flat:{attack:5,knockback:4,defense:-3,balance:-4,stamina:-7,mobility:5,burst:1},
    "Low Flat":{attack:6,knockback:5,defense:-4,balance:-4,stamina:-8,mobility:6,burst:1},
    Rush:{attack:2,knockback:1,defense:-1,balance:-1,stamina:-2,mobility:3,burst:1},
    "Low Rush":{attack:3,knockback:2,defense:-1,balance:-1,stamina:-3,mobility:4,burst:1},
    Kick:{attack:2,knockback:2,defense:-1,balance:1,stamina:-2,mobility:2,burst:0},
    Quake:{attack:5,knockback:5,defense:-4,balance:-5,stamina:-8,mobility:3,burst:0},
    Point:{attack:0,knockback:0,defense:-1,balance:1,stamina:1,mobility:2,burst:1},
    Level:{attack:1,knockback:1,defense:-1,balance:0,stamina:0,mobility:3,burst:1},
...
function easeStatDelta(stat,delta){
    const n=Number(stat)||70;
    const d=Number(delta)||0;
    if(!d) return n;
    const t=Math.max(0,Math.min(1,(n-60)/39));
    if(d>0){
        const scale=1-0.78*t*t;
        return n+d*scale;
    }
    const scale=0.45+0.55*t;
    return n+d*scale;
}
function addEasedDelta(target,delta){
    if(!delta) return target;
...
function getBladeGeometry(blade){
    ...
    const mapped=BLADE_GEOMETRY[Object.keys(BLADE_GEOMETRY).find(n=>n.toLowerCase().replace(/ /g,"_")===key)];
    if(mapped) return mapped;
    return inferBladeGeometry(blade);
}
function inferBladeGeometry(blade){
    const shape=String(blade?.physics?.contactShape||"").toLowerCase();
    const dist=String(blade?.physics?.weightDistribution||"").toLowerCase();
    const weight=Number(blade?.weight)||35;
    let symmetry="round";
    if(shape.includes("claw")) symmetry="claw";
    else if(shape.includes("elliptical")) symmetry="elliptical";
    else if(shape.includes("tri")||shape.includes("three")) symmetry="three";
    else if(shape.includes("oval")) symmetry="oval";
    else if(shape.includes("multi")) symmetry="multi";
    else if(dist.includes("forward")||shape.includes("forward")) symmetry="forward";
    else if(shape.includes("smash")||shape.includes("upper")) symmetry="three";
    else if(shape.includes("round")) symmetry="round";
    const massBias=dist.includes("forward")?"forward":dist.includes("outer")?"outer":"balanced";
    return {
        symmetry,
        hooks:shape.includes("hook"),
        massBias,
...
function getHeightDeltaV58(height){
    ...
    return {attack:1,knockback:0,defense:0,balance:0,stamina:0,mobility:0,burst:2};
}
function getAlignDeltaV58(blade,ratchet){
    ...
    if(g.symmetry==="three" && n===9 && g.hooks){d.attack+=1;d.knockback+=1;}
    ...
    if(g.symmetry==="oval" && (n===6||n===9)){d.stamina+=1;d.balance+=1;}
    if(g.symmetry==="multi" && n===5){d.balance+=2;}
    if(g.symmetry==="multi" && n===9){d.stamina+=1;}
    if(g.massBias==="forward" && n===1){d.attack+=1;d.knockback+=1;}
    if(n===4){d.burst-=1;}
    return d;
}
function getBitDeltaV58(bit){
    const named=BIT_STAT_DELTA[bit?.name];
    if(named) return {...named};
    const bp=getBitPhysicalProfileV56(bit);
    const scale={attack:5,knockback:4,defense:5,mobility:8,balance:6,stamina:8,burst:4};
    const out={};
...
function getSpecialCaseDeltaV58(blade,ratchet,bit){
    ...
    if(type==="Stamina" && g.symmetry==="round" && n===1 && h===60 && bitName==="Hexa"){
        d.stamina+=2;d.balance+=3;d.defense+=2;
    }
...
function applyComboModifiers(base,blade,ratchet,bit){
    ...
    addEasedDelta(stats,RATCHET_SIDE_DELTA[Number(ratchet?.number)]||RATCHET_SIDE_DELTA[3]);
    addEasedDelta(stats,getHeightDeltaV58(ratchet?.height));
    addEasedDelta(stats,getAlignDeltaV58(blade,ratchet));
    if(bit) addEasedDelta(stats,getBitDeltaV58(bit));
    if(bit) addEasedDelta(stats,getSpecialCaseDeltaV58(blade,ratchet,bit));
...
function renderHowTo(){
    ...
    <main class="menu howto">
        ...
        <section class="menu-card howto-card" id="ht-combat">
            ...
            ${howToKitLine("earthquake")}
            ${howToKitLine("pegasus-blast")}
        ...
        <section class="menu-card howto-card" id="ht-score">
            ...
            <p>The scoreboard is 0 VS 0 with "first to 7" underneath. Rogue prints the match number there. When someone hits 7, Quick Play and Rogue open a Match Summary. RPM Damage pays more as you deal more (denser bands at high totals). Ability scores kit damage, restore (Hurricane), and Over/Xtreme pockets caused by an ability shove. The Finish Chain lists the best linked point in order — Big Impact, X-Rail, Finish — and that stack sets the match multiplier on base score. Dashes and X-Rail rides list at +0. Game quality uses the Horrible / Bad / Okay / Good / Perfect ladder from how you played and what you scored.</p>
...
function getRatchetGeometryFitV56(blade,ratchet){
    ...
    if(n===1){
        ...
        else if(type==="Stamina" && g.symmetry==="round") fit+=.14;
...
function calculateOverallScoreV58(blade,ratchet,bit,stats){
    ...
    const attackBit=["Flat","Low Flat","Rush","Low Rush","Kick","Quake"].includes(bitName);
    const controlAttack=["Rush","Low Rush"].includes(bitName);
    const rawAttack=["Flat","Low Flat","Quake"].includes(bitName);
    const tankBit=["Ball","Orb","Hexa","Needle","Wedge"].includes(bitName);

    if(h===60) score+=0.35;
    if(h===80) score-=1.6;
    if(h===80 && tankBit) score-=2.0;
    if(n===4) score-=3.2;

    if(type==="Attack" && n===1 && h===60 && controlAttack) score+=1.8;
    else if(type==="Attack" && n===1 && h===60 && rawAttack) score+=0.8;
    else if(type==="Attack" && n===1 && h===60) score+=0.5;
    if(g.symmetry==="three" && n===3 && h===60) score+=2.5;
    if(g.hooks && n===9 && h===60) score+=1.8;
    if(g.wantsWeight && n===7 && h===60) score+=2.6;
    if(g.symmetry==="round" && (n===9||n===6) && tankBit) score+=1.8;
    if(g.symmetry==="round" && n===1 && bitName==="Hexa") score+=2.4;
    if(g.massBias==="forward" && n===1 && h===60 && attackBit) score+=1.6;
    if(g.massBias==="forward" && n===5 && h===60) score+=1.5;
    if(g.symmetry==="claw" && n===3 && h===60) score+=1.3;
    if(g.symmetry==="elliptical" && n===3 && h===60) score+=1.2;
    if(g.symmetry==="oval" && (n===6||n===9) && tankBit) score+=1.2;
    if(g.symmetry==="multi" && n===5 && h===60) score+=1.0;
    if(n===5 && g.symmetry==="round" && (bitName==="Ball"||bitName==="Orb")) score-=2.4;
    if(type==="Stamina" && attackBit) score-=1.2;
    if(type==="Defense" && rawAttack) score-=1.4;
    if(type==="Attack" && tankBit) score-=0.8;

    if(type==="Attack" && n===1 && h===60 && bitName==="Ball") score+=1.4;
    if(blade.rogueBoss && n===1 && h===60 && bitName==="Ball") score+=0.8;
...
function calculateComboStats(blade,ratchet,bit){
    ...
    const deltaFromBlade=statDeltaMap(base,stats);
...
function showLetItRip(){
    ...
    renderNewBattle();
    paintTiltChip(NEW_BATTLE.player);
...
function paintTiltChip(s){
    ...
    const leftFrac=tiltTimeLeft(s);
    if(fade<=0.03 || !(s?.launchTiltBias>0)){
        el.hidden=true;
        el.style.opacity="";
        return;
    }
    const copy=tiltStatCopy(s.launchTiltKind);
    el.hidden=false;
    el.style.opacity=String(Math.max(0.38,leftFrac));
    const plus=el.querySelector(".tilt-plus");
    const minus=el.querySelector(".tilt-minus");
    const ring=el.querySelector(".tilt-chip-fill");
    if(plus) plus.textContent=copy.plus;
    if(minus) minus.textContent=copy.minus;
    if(ring){
        const c=2*Math.PI*14;
        ring.style.strokeDasharray=String(c);
        ring.style.strokeDashoffset=String(c*(1-leftFrac));
    }
}
...
function tiltDurationFromAngle(angle){
    if(angle==="Hard Tilt") return 3.3;
    if(angle==="Slight Tilt") return 2.5;
    return 0;
}

function tiltStatAmounts(angle){
    if(angle==="Hard Tilt") return {hit:2,hold:2};
    if(angle==="Slight Tilt") return {hit:1,hold:1};
    return {hit:0,hold:0};
}
function tiltStatCopy(angle){
    const n=tiltStatAmounts(angle);
    if(!(n.hit||n.hold)) return {plus:"",minus:""};
    return {
        plus:`+${n.hit} ATK · +${n.hit} KB`,
        minus:`−${n.hold} DEF · −${n.hold} MOB`
    };
}

function tiltTimeLeft(s){
    const max=Number(s?.launchTiltMax)||0;
    const left=Number(s?.launchTiltLeft)||0;
    if(!(max>0)) return 0;
    return Math.max(0,Math.min(1,left/max));
}
...
function tiltAngleBlurb(angle){
    if(angle==="Hard Tilt") return "+2 ATK/KB · −2 DEF/MOB · 3.3s · harder rail hook";
    if(angle==="Slight Tilt") return "+1 ATK/KB · −1 DEF/MOB · 2.5s · rail can creep";
          ...
          <div class="battle-dock">
            <div class="battle-fighters">
              <div class="battle-hud-card battle-hud-player">
                <div id="playerTiltChip" class="tilt-chip" hidden>
                  <svg class="tilt-chip-ring" viewBox="0 0 36 36" aria-hidden="true">
                    <circle class="tilt-chip-track" cx="18" cy="18" r="14"></circle>
                    <circle class="tilt-chip-fill" cx="18" cy="18" r="14"></circle>
                  </svg>
                  <span class="tilt-chip-deltas"><b class="tilt-plus"></b><i class="tilt-minus"></i></span>
                </div>
...
function getBattleStatPoints(s,key,fallback=70){
    ...
    const amounts=tiltStatAmounts(s?.launchTiltKind);
    const tiltDelta=(key==="attack"||key==="knockback")?tilt*amounts.hit
        :(key==="defense"||key==="mobility")?-tilt*amounts.hold
        :0;
    ...
    return Math.max(50,Math.min(102,total));
}
...
function matchStatCounter(key){
    if(key==="attack") return "defense";
    if(key==="defense") return "attack";
    if(key==="knockback") return "balance";
    if(key==="balance") return "knockback";
    return key;
}
...
/*
  Stats set potential, not a flat tax. Equal cards play at STAT_PAR
...
function matchScaledStat(myPoints,theirPoints){
    const rogue=typeof SpinWarsRogue!=="undefined" && SpinWarsRogue.isActive();
    const mine=rogue
        ? Math.max(1,Number(myPoints)||70)
        : Math.max(50,Math.min(99,Number(myPoints)||70));
    const theirs=rogue
        ? Math.max(1,Number(theirPoints)||70)
        : Math.max(50,Math.min(99,Number(theirPoints)||70));
    const mean=Math.max(1,(mine+theirs)*0.5);
    const gap=Math.tanh((mine-theirs)/45)*0.58;
    const equalWeight=1+newBattleClamp((mean-70)*0.0007,-0.03,0.06);
    return newBattleClamp(
        (STAT_PAR+gap)*equalWeight,
        rogue?0.48:0.52,
        rogue?1.52:1.48
    );
}
...
/*
  A graze barely spends the stat budget (~0.12–0.20). A mouth-force
  smash can spend most of it (~1.0). Apply only to the gap from par:
  effective = STAT_PAR + (scaled - STAT_PAR) * spend.
*/
function clashImpactSpend(effectiveImpact,directness){
    const smash=newBattleClamp((Number(effectiveImpact)||0)/0.028,0,1);
    const graze=newBattleClamp(Number(directness)||0,0,1);
    return newBattleClamp(
        0.12+Math.pow(graze,1.35)*0.38+Math.pow(smash,1.40)*0.52,
        0.12,
        1
    );
}

function spendMatchStat(scaled,spend){
    const s=Number(scaled);
    const p=Number.isFinite(s)?s:STAT_PAR;
    const t=newBattleClamp(Number(spend)||0,0,1);
    return STAT_PAR+(p-STAT_PAR)*t;
}
...
function newPhysicsCollision(dt){
    ...
    */
    /*
      Card Defense / Balance are the high-RPM wall. As remaining RPM
      drops, that wall comes down so dying Beys take a little more RPM
      loss (Defense) and knock (Balance) instead of stalling in a
      low-spin chip war.
    */
    const liveWall=(cardStat,rpm)=>newBattleClamp(
        cardStat*(0.40+0.60*Math.pow(Math.max(0.08,rpm),1.05)),
        0,1.05
    );
    let pDef=liveWall(pCardDef,pRPM);
    let cDef=liveWall(cCardDef,cRPM);
    let pBalLive=liveWall(pBal,pRPM);
    let cBalLive=liveWall(cBal,cRPM);
    ...
    */
    ...
    /*
      Spend the stat budget on this contact. Grazes keep both Beys near
      par. A real smash can spend most of the Attack/Defense and
      Knockback/Balance gap. Live RPM fade still applies after spend so
      a dying wall does not get "refunded" toward par on a light tap.
    */
    const impactSpend=clashImpactSpend(effectiveImpact,directness);
    const pAtkHit=spendMatchStat(pAttack,impactSpend);
    const cAtkHit=spendMatchStat(cAttack,impactSpend);
    const pKBHit=spendMatchStat(pKB,impactSpend);
    const cKBHit=spendMatchStat(cKB,impactSpend);
    pDef=liveWall(spendMatchStat(pCardDef,impactSpend),pRPM);
    cDef=liveWall(spendMatchStat(cCardDef,impactSpend),cRPM);
    pBalLive=liveWall(spendMatchStat(pBal,impactSpend),pRPM);
    cBalLive=liveWall(spendMatchStat(cBal,impactSpend),cRPM);
    ...
    */
    ...
    const pDefenseSoak=(pHitLikeAttack && !cAttackBit
        ? (1.04+(1-cBalLive)*0.26+(1-cDef)*0.08)
        : (0.92+(1-cBalLive)*0.28+(1-cDef)*0.08))*(1+(1-cRPM)*0.20);
    const cDefenseSoak=(cHitLikeAttack && !pAttackBit
        ? (1.04+(1-pBalLive)*0.26+(1-pDef)*0.08)
        : (0.92+(1-pBalLive)*0.28+(1-pDef)*0.08))*(1+(1-pRPM)*0.20);
    let pKnockRaw=Math.max(
        0.007+pKBHit*0.010+pRPM*0.004,
        (bounceSep*0.42+pSpinPower+pMomentum*0.22+pForce*0.014)*
        pDefenseSoak*
        (1.02+newBattleClamp(momentumFactor/2.4,0,0.16))
    );
    let cKnockRaw=Math.max(
        0.007+cKBHit*0.010+cRPM*0.004,
    ...
    */
    ...
    let recoilP=pKnockback*(0.04+0.05*pDef+0.08*pBalLive);
    let recoilC=cKnockback*(0.04+0.05*cDef+0.08*cBalLive);
    if(pHitLikeAttack && !cHitLikeAttack){
        recoilC*=0.36;
        recoilP*=0.88;
    }else if(cHitLikeAttack && !pHitLikeAttack){
    ...
    */
    /*
      ...
      not a second melt stat. Match-relative Attack vs Defense is the
      RPM gap; Knockback vs Balance is the shove. Both only spend as
      hard as this contact was.
    */
    const pStatDamageFactor=0.45+0.66*pAtkHit;
    const cStatDamageFactor=0.45+0.66*cAtkHit;
