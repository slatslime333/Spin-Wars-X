...
/*======================================================================
 SPIN WARS X — NEW BATTLE ENGINE
 Version: 1.0 FOUNDATION

 This section intentionally supersedes the old zone/turn battle engine.
 The old functions remain above as legacy reference code, but none of them
 are called by the new battle loop.

 Architecture:
   launch → continuous simulation → physical interaction → commentary
          → decision interrupt (later) → physical result → finish

 First integrated layer:
   continuous movement + RPM + momentum + stability + tilt + collision

 X Rail / pockets / Xtreme / burst / decisions / CPU brain are added in
 subsequent layers. They do NOT get simulated as fake zone events here.
======================================================================*/

(function installNewBattleEngine(){
"use strict";
...
const PHYS={
    // Coordinates are normalized to the actual SVG stadium.
    cx:500, cy:420,
    rx:326, ry:292,
    wallRx:326, wallRy:292,
    beyRadius:13,
...
function getProfile(side){
    const s=Game[side]||{};
    const combo=(typeof getBattleCombo==="function") ? getBattleCombo(side) : null;
    const st=combo?.stats || s.stats || {};
    const bit=(typeof getBitPhysics==="function") ? getBitPhysics(side) : null;
    const b=s.blade||{};
    const r=s.ratchet||{};
    return {
        attack:num(st.attack,70),
        knockback:num(st.knockback,70),
        defense:num(st.defense,70),
        balance:num(st.balance,70),
        stamina:num(st.stamina,70),
        mobility:num(st.mobility,70),
        burst:num(st.burst,70),
        weight:num(b.weight,38),
        type:b.type||"Balance",
        spin:b.spin||"Right",
        height:num(r.height,70),
        bitName:s.bit?.name||"Point",
        bitMovement:num(bit?.movement,58),
        bitControl:num(bit?.control,82),
        bitDrain:num(bit?.spinDrain,0.74),
        bitRecovery:num(bit?.recovery,72),
        railAffinity:num(bit?.xRailAffinity,48),
        centerAffinity:num(bit?.centerAffinity,70)
    };
}

function qualityFactor(q){
    return {Horrible:.68,Bad:.82,Okay:1,Good:1.08,Perfect:1.15}[q]||1;
}

function launchDirection(side,launch){
    const sign=side==="player"?1:-1;
    const q=qualityFactor(launch?.quality);
    let vx=sign*q, vy=0;
    const angle=launch?.angle||"Flat";
    const technique=launch?.technique||"Center";

    // Starting velocity is an intent, not a scripted route.
    if(technique==="Direct Clash"){
        vx=sign*q*1.28;
        vy=(Math.random()-.5)*.20;
    }else if(technique==="Wide Circle"){
        vx=sign*q*.86;
        vy=side==="player"?-0.34:0.34;
    }else if(technique==="Drop Launch"){
        vx=sign*q*.45;
        vy=side==="player"?-.60:.60;
    }else if(String(technique).includes("Rail")){
        vx=sign*q*1.05;
        vy=side==="player"?-.16:.16;
    }else{
        vx=sign*q;
        vy=(Math.random()-.5)*.18;
    }
    ...
    if(angle==="Slight Tilt"){
        vy+=side==="player"?-0.10:0.10;
        vx*=.93;
    }
    if(angle==="Hard Tilt"){
        vy+=side==="player"?-0.20:0.20;
        vx*=.86;
    }
...
function createPhysicalState(side){
    const p=getProfile(side);
    const launch=Game[side]?.launch||{};
    const startX=side==="player"?-0.62:0.62;
    const d=launchDirection(side,launch);
    const launchSpeed=.0010 + p.bitMovement*.000012;
    const rpm=clamp(
        .82+(p.stamina-70)*.0035+(num(launch.spinBonus,0)*.001),
        .62,1
    );
    const initialTilt=
        launch.angle==="Hard Tilt" ? 0.14 :
        launch.angle==="Slight Tilt" ? 0.07 : 0.025;
    ...
    return {
        ...
        vy:d.y*launchSpeed,
        rpm,
        momentumX:d.x*p.weight*.035,
        momentumY:d.y*p.weight*.035,
        tilt:initialTilt,
        stability:clamp(.72+(p.balance-70)*.004,0.28,1),
        spinDirection:p.spin,
        profile:p,
        rail:{active:false,progress:0,direction:1},
        finish:null,
        contactCooldown:0,
        intent:null,
        behavior:"circling",
...
function worldToSvg(s){
    return {x:PHYS.cx+s.x*PHYS.rx,y:PHYS.cy+s.y*PHYS.ry};
}

function speed(s){return Math.hypot(s.vx,s.vy);}

function applyNaturalPhysics(s,dt){
    const p=s.profile;
    const v=speed(s);
    const rpmFactor=.38+.62*s.rpm;
    const control=clamp(.65+p.bitControl*.003+s.stability*.16,.55,1.12);
    const mobility=clamp(.62+p.bitMovement*.0035,.65,1.18);

    // Bit surface behavior: movement costs RPM; high-friction/attack Bits
    // spend more spin while moving.
    const movementDrain=(.00034+p.bitDrain*.000075)*dt*60*(1+v*130);
    s.rpm=clamp(s.rpm-movementDrain,0,1);

    const drag=clamp(.989-p.bitMovement*.000015,.982,.990);
    s.vx*=Math.pow(drag,dt*60);
    s.vy*=Math.pow(drag,dt*60);

    // Very small continuous steering toward the Bit's natural tendency.
    // This creates behavior without choosing zones.
    const centerPull=p.centerAffinity/100*.0000012;
    s.vx+=(-s.x)*centerPull*dt*60;
    s.vy+=(-s.y)*centerPull*dt*60;

    const moveScale=mobility*control*rpmFactor;
    s.x+=s.vx*dt*60*moveScale;
    s.y+=s.vy*dt*60*moveScale;

    // Momentum follows actual velocity.
    s.momentumX=s.momentumX*.94+s.vx*p.weight*.06;
    s.momentumY=s.momentumY*.94+s.vy*p.weight*.06;

    // Stability and tilt are separate states.
    const lateral=v*1.8;
    const recovery=p.bitRecovery/100*(.00065+Math.max(0,s.rpm-.35)*.0005);
    const stress=lateral*.0009 + Math.max(0,s.tilt)*.00035;
    s.stability=clamp(s.stability+recovery*dt*60-stress*dt*60,0,1);

    const tiltPressure=Math.max(0,(1-s.stability))*0.0015;
    s.tilt=clamp(s.tilt+tiltPressure*dt*60-recovery*.65*dt*60,0,.48);

    if(s.contactCooldown>0) s.contactCooldown=Math.max(0,s.contactCooldown-dt);
}

function boundaryResponse(s){
    const nx=s.x/PHYS.arenaInner;
    const ny=s.y/PHYS.arenaInner;
    const normalized=Math.hypot(nx,ny);
    if(normalized<=1) return false;

    // Soft stadium wall. Finish zones are not implemented yet, so a wall
    // contact is currently a physical bounce rather than an automatic finish.
    const px=s.x/normalized, py=s.y/normalized;
    s.x=px*PHYS.arenaInner;
    s.y=py*PHYS.arenaInner;

    const dot=s.vx*px+s.vy*py;
    if(dot>0){
        s.vx-=dot*px*1.65;
        s.vy-=dot*py*1.65;
        s.vx*=.76;
        s.vy*=.76;
        s.stability=clamp(s.stability-.025,0,1);
        s.tilt=clamp(s.tilt+.018,0,.48);
    }
    return true;
}

function resolveCollision(a,b){
    if(a.contactCooldown>0 || b.contactCooldown>0) return null;

    const dx=b.x-a.x, dy=b.y-a.y;
    const dist=Math.hypot(dx,dy);
    if(dist>PHYS.collisionDistance/PHYS.rx) return null;

    const n=dist<.00001?{x:a.side==="player"?1:-1,y:0}:{x:dx/dist,y:dy/dist};
    const rvx=b.vx-a.vx, rvy=b.vy-a.vy;
    const relative=rvx*n.x+rvy*n.y;
    if(relative>0) return null;

    const pa=a.profile, pb=b.profile;
    const ma=Math.max(20,pa.weight), mb=Math.max(20,pb.weight);
    const restitution=clamp(.56+(pa.balance+pb.balance-140)*.0015,.42,.72);
    ...
    const impact=Math.abs(relative)*((ma+mb)*.5)*1000;
    const attacker=a.momentumX*a.vx+a.momentumY*a.vy > b.momentumX*b.vx+b.momentumY*b.vy ? a:b;
    const defender=attacker===a?b:a;
    const ap=attacker.profile, dp=defender.profile;

    const angleQuality=clamp(Math.abs(relative)*70,0,1);
    const attackFactor=.72+ap.attack*.0055;
    const knockFactor=.72+ap.knockback*.0055;
    const defenseFactor=.72+dp.defense*.0052;
    const balanceFactor=.72+dp.balance*.0048;

    const effective=impact*attackFactor*knockFactor/(defenseFactor*balanceFactor);
    const rpmDamage=clamp(effective*.0000028,0,.16);
    const stabilityDamage=clamp(effective*.0000034,0,.20);

    defender.rpm=clamp(defender.rpm-rpmDamage,0,1);
    defender.stability=clamp(defender.stability-stabilityDamage,0,1);
    defender.tilt=clamp(defender.tilt+stabilityDamage*.65+angleQuality*.018,0,.48);

    // Recoil is physical and applies to the attacker too.
    const recoil=.00055*(1+ap.attack*.004)*(1-ap.balance*.002);
    attacker.vx-=n.x*recoil;
    attacker.vy-=n.y*recoil;

    a.contactCooldown=.10;
    b.contactCooldown=.10;
...
function checkSpinFinish(){
    const p=Game.battle.physics;
    if(!p) return null;
    if(p.player.rpm<=0.001 && p.cpu.rpm<=0.001) return "draw";
    if(p.player.rpm<=0.001) return "cpu";
    if(p.cpu.rpm<=0.001) return "player";
    return null;
}
...
function updateBattleVisuals(){
    ...
    if(!p||!c) return;
    const pe=document.getElementById("playerBey");
    const ce=document.getElementById("cpuBey");
    const pl=document.getElementById("playerBeyLabel");
    const cl=document.getElementById("cpuBeyLabel");
    if(!pe||!ce) return;
    const pp=worldToSvg(p), cp=worldToSvg(c);
    pe.setAttribute("cx",pp.x); pe.setAttribute("cy",pp.y);
    ce.setAttribute("cx",cp.x); ce.setAttribute("cy",cp.y);
    pe.setAttribute("r",String(12+Math.min(3,p.tilt*7)));
    ce.setAttribute("r",String(12+Math.min(3,c.tilt*7)));
    if(pl){pl.setAttribute("x",pp.x);pl.setAttribute("y",pp.y-20);pl.textContent="YOU";}
    if(cl){cl.setAttribute("x",cp.x);cl.setAttribute("y",cp.y-20);cl.textContent="CPU";}
}

function syncLegacyDisplayState(){
    const p=Game.battle.physics?.player, c=Game.battle.physics?.cpu;
    if(!p||!c) return;
    // These fields exist only so the existing HUD can display meaningful
    // values while the new engine is being integrated. They do not control
    // the simulation.
    for(const [key,s] of [["player",p],["cpu",c]]){
        const b=Game.battle[key];
        b.spin=clamp(s.rpm*100,0,100);
        b.balance=clamp(s.stability*100,0,100);
        b.momentum=clamp(Math.hypot(s.momentumX,s.momentumY)*2,-100,100);
        b.zone=null;
    }
}

function commentary(line){
    const b=Game.battle;
    b.liveCommentary=line;
    const el=document.getElementById("liveCommentary");
    if(el) el.textContent=line;
}

function showImpactFlash(result){
    const g=document.getElementById("impactEffect");
    const txt=document.getElementById("impactText");
    if(!g||!txt) return;
    const p=Game.battle.physics[result.attacker], c=Game.battle.physics[result.defender];
    const a=worldToSvg(p), d=worldToSvg(c);
    const x=(a.x+d.x)/2, y=(a.y+d.y)/2;
    g.setAttribute("transform",`translate(${x-500} ${y-420})`);
    txt.textContent=result.impact>75?"BIG HIT!":"HIT!";
    g.setAttribute("opacity","1");
    setTimeout(()=>g.setAttribute("opacity","0"),260);
}

function makeBattleScreen(){
    const app=document.getElementById("app");
    app.innerHTML=`
    <div class="background"></div>
    <main class="menu">
      <section class="menu-card" style="max-width:560px;padding:12px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div><h1 style="margin:0;">BATTLE</h1><div id="battleClock" style="font-size:9px;opacity:.55;">0.0s</div></div>
          <div style="font-size:11px;"><strong>YOU ${Game.battle.playerScore||0}</strong> — <strong>CPU ${Game.battle.cpuScore||0}</strong></div>
        </div>
        <hr>
        ${renderStadium()}
        <div style="margin-top:6px;padding:9px 10px;border-radius:9px;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.08);">
          <div style="font-size:9px;opacity:.6;letter-spacing:.7px;margin-bottom:3px;">COMMENTARY</div>
...
function updateLiveStats(){
    const el=document.getElementById("liveBattleStats");
    const p=Game.battle.physics?.player, c=Game.battle.physics?.cpu;
    if(!el||!p||!c) return;
    const box=(name,s)=>`<div style="padding:7px 8px;border-radius:8px;background:rgba(255,255,255,.04);font-size:9px;"><strong>${name}</strong><br>RPM ${Math.round(s.rpm*100)}% · STAB ${Math.round(s.stability*100)}%<br>TILT ${Math.round(s.tilt*100)}% · SPD ${(speed(s)*1000).toFixed(1)}</div>`;
    el.innerHTML=box(Game.player.blade.name,p)+box(Game.cpu.blade.name,c);
}

function finishBattle(winner,finish="Spin Finish"){
    const b=Game.battle;
    if(b.finished) return;
    b.finished=true;
    b.matchFinished=false;
    b.winner=winner;
    b.finish=finish;
    b.finishPoints=finish==="Spin Finish"?1:finish==="Over Finish"?2:3;

    if(winner==="player") b.playerScore=(b.playerScore||0)+b.finishPoints;
    if(winner==="cpu") b.cpuScore=(b.cpuScore||0)+b.finishPoints;
    b.matchFinished=b.playerScore>=b.pointsToWin || b.cpuScore>=b.pointsToWin;

    commentary(winner==="player"?"SPIN FINISH! You outlast the CPU.":winner==="cpu"?"SPIN FINISH! The CPU outlasts you.":"DRAW!");
...
function showBattleResultScreen(){
    ...
    const winnerName=winner==="player"?Game.player.blade.name:Game.cpu.blade.name;
    const points=b.finishPoints||1;
    const app=document.getElementById("app");
    app.innerHTML=`
    <div class="background"></div><main class="menu"><section class="menu-card" style="max-width:560px;text-align:center;">
      <h1>${b.finish||"BATTLE RESULT"}</h1><hr>
      <h2>${winnerName} WINS</h2>
      <p>+${points} POINT${points===1?"":"S"}</p>
      <p style="font-size:18px;"><strong>YOU ${b.playerScore}</strong> — <strong>CPU ${b.cpuScore}</strong></p>
      <p style="opacity:.7;font-size:11px;">${b.matchFinished?"MATCH COMPLETE.":"NEXT BATTLE SEQUENCE READY."}</p>
      <button class="menu-btn gold" id="battleContinue">${b.matchFinished?"VIEW MATCH RESULT":"CONTINUE"}</button>
    </section></main>`;
    document.getElementById("battleContinue").onclick=()=>{
        if(b.matchFinished) showMatchSummary();
        else { b.round++; showLaunchScreen(); }
    };
}

function startNewPhysicalBattle(){
    const b=Game.battle;
    b.engineMode="physics_v1";
    b.finished=false;
    b.winner=null;
    b.finish=null;
    b.finishPoints=0;
    b.decisionActive=false;
    b.pendingDecision=null;
    b.matchStarted=true;
    b.pointsToWin=b.pointsToWin||7;
    b.round=b.round||1;
    b.physics={
        version:"1.0-foundation",
        time:0,
        active:true,
        player:createPhysicalState("player"),
        cpu:createPhysicalState("cpu")
    };
    makeBattleScreen();
    commentary(`${Game.player.blade.name} and ${Game.cpu.blade.name} are off!`);
    runPhysics();
}
...
function runPhysics(){
    const b=Game.battle;
    const p=b.physics;
    if(!p||!p.active||b.finished) return;
    let last=performance.now();
    let lastUi=last;
    let lastComment=last;
    let collisionCount=0;

    function frame(now){
        if(!b.physics?.active || b.finished) return;
        const dt=Math.min(PHYS.maxDt,Math.max(.001,(now-last)/1000));
        last=now;
        p.time+=dt;

        applyNaturalPhysics(p.player,dt);
        applyNaturalPhysics(p.cpu,dt);

        boundaryResponse(p.player);
        boundaryResponse(p.cpu);

        const hit=resolveCollision(p.player,p.cpu);
        if(hit){
            collisionCount++;
            hit.index=collisionCount;
            b.lastEvent="collision";
            commentary(hit.impact>75?"WHAT A HIT!":"They collide!");
            showImpactFlash(hit);
        }
        ...
        if(now-lastUi>100){
            updateLiveStats();
            const clock=document.getElementById("battleClock");
            if(clock) clock.textContent=p.time.toFixed(1)+"s";
            lastUi=now;
        }

        if(now-lastComment>2200 && !hit){
            const ps=p.player, cs=p.cpu;
            if(ps.tilt>.22 || cs.tilt>.22) commentary("They're starting to lose stability!");
            else if(ps.rpm<.45 || cs.rpm<.45) commentary("Spin is starting to fade.");
            else commentary("They're circling and looking for an opening.");
            lastComment=now;
        }

        const winner=checkSpinFinish();
        if(winner){
            p.active=false;
            finishBattle(winner,"Spin Finish");
            return;
        }
...
// -------------------------------------------------------------------
// Replace the old battle entry points. Menus, drafts and launch choices
// above remain intact.
// -------------------------------------------------------------------
...
window.continueBattleSequence=function(){
    if(Game.battle.matchFinished) return showMatchSummary();
    Game.battle.round=(Game.battle.round||1)+1;
    showLaunchScreen();
};

window.decideNextBattleStep=function(){
    // The new engine does not use turns/ticks as decisions.
    // Decisions will later interrupt the live RAF simulation.
    if(Game.battle.engineMode==="physics_v1" && Game.battle.physics?.active) return;
    startNewPhysicalBattle();
};

window.simulateBattleRound=function(){
    // Legacy compatibility entry point. It intentionally does not simulate a
    // turn; the requestAnimationFrame loop is authoritative.
    if(Game.battle.engineMode==="physics_v1") return;
    startNewPhysicalBattle();
};

window.checkBattleFinish=function(){
    const winner=checkSpinFinish();
    if(winner){finishBattle(winner,"Spin Finish");return true;}
    return false;
};

window.renderBeys=function(){
    if(Game.battle?.physics){ updateBattleVisuals(); return; }
};

// A clean live battle screen replaces the old sequence viewer for the new
// engine. History will be reintroduced after physical events are authoritative.
window.renderBattleSequence=function(){
    if(Game.battle?.engineMode==="physics_v1"){
        updateBattleVisuals();
        updateLiveStats();
        return;
    }
...
// The old launch animation is a zone-path animation, so do not use it in the
// new engine. Preserve the launch choice/quality, then hand off directly to
// the physical simulation.
window.showLaunchExecution=function(){
    const app=document.getElementById("app");
    app.innerHTML=`<div class="background"><main class="menu"><section class="menu-card" style="text-align:center;max-width:520px;">
      <h1>LAUNCH READY</h1><hr>
      <h2>${Game.player.blade.name}</h2>
      <p>${Game.player.launch.angle} · ${Game.player.launch.technique}</p>
      <div style="margin:12px 0;padding:12px;border-radius:10px;background:rgba(255,255,255,.05);">
        <div style="font-size:9px;opacity:.6;">LAUNCH QUALITY</div>
        <strong style="font-size:24px;">${Game.player.launch.quality}</strong>
      </div>
      <p style="font-size:11px;opacity:.7;">The launch is an initial physical condition. The stadium now determines what happens.</p>
      <button class="menu-btn gold" id="launchQuality">QUALITY · LAUNCH</button>
      <button class="menu-btn silver" id="launchRisk">RISK · REROLL</button>
    </section></main></div>`;

    document.getElementById("launchQuality").onclick=()=>{
        Game.player.launch.gamble=false;
        applyLaunchQuality("player");
        validateLaunch("player");
        generateCPULaunch();
    };
    document.getElementById("launchRisk").onclick=()=>{
        Game.player.launch.gamble=true;
        Game.player.launch.quality=rollRiskQuality();
        applyLaunchQuality("player");
        validateLaunch("player");
...
// New launch execution should still use the existing CPU selection logic.
