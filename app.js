...
function handleTacticalChoice(event){
    ...
    const intent=btn?.dataset?.tacticalIntent;
...
function chooseDynamicMove(intent){
    if(Game.battle.finished || Game.battle.decisionResolving) return false;
    ...
    // Lock the decision before any state/render work. There is now exactly one
    // live decision transaction and no intermediate battle screen render.
    Game.battle.decisionResolving=true;
    ...
    try{
        ...
        // Resolve first. The resolver owns the next battle state; it will
        // create the decision-result history entry and render exactly once.
        resolvePlayerIntent();
        return true;
    }catch(error){
        console.error("Tactical decision resolution failed:",error);

        // Never leave a decision locked with no path forward.
        ...
        Game.battle.decisionResolving=false;
    }
}
...
function showIntentResult(intent,cpuIntent,result,text){
    ...
    try{
        ...
        console.error("Battle result render failed:",error);
    }

    // Every tactical result, including X-Rail decisions, returns to the same
    // automatic battle engine. This prevents a successful decision from
    // becoming a dead-end screen.
...
function decideNextBattleStep(){
    ...
    // Tactical decisions pause the automatic engine. Any stale timeout that
    // fires while the player is choosing must not create another decision or
    // advance the battle underneath the visible choice.
    if(Game.battle.decisionActive || Game.battle.decisionResolving){
        return;
    }
...
// Keep the resolver available for an already-open/cached page, while the live
// renderer uses direct handlers on the freshly-created tactical buttons.
