...
function pushBey(bey){
    ...
    for(let i=0;i<pushes;i++){
        ...
        const preferDanger=force>72 && (attackerKnockback>78 || attackerAttack>80);
        const scored=destinations.map(zone=>{
            let score=Math.random()*8;
            if(zone===opponent.zone) score-=8;
            if(dangerZones.includes(zone)){
                score += preferDanger ? 24 : -6;
                if(zone==="XtremeZone") score += (force>105 && attackerKnockback>88) ? 12 : -10;
                if((zone==="LeftPocket" || zone==="RightPocket") && attackPower>84) score += 8;
            }
...
function checkStadiumDanger(bey){
    ...
    if(zone==="XtremeZone"){
        ...
        const aggressive=(power>=82 || incoming>=75);
        const chance=Math.max(28,Math.min(92,
            30 + (power-70)*0.35 + (incoming-55)*0.28 + momentum*0.12
        ));
        // Xtreme should be rare, but aggressive/high-KB builds should see it.
        if(aggressive && Math.random()*100 < chance){
...
function decideNextBattleStep(){
    ...
    const dangerous=
        ...
        ["LeftPocket","RightPocket","LeftRail","RightRail","XRailExit"].includes(Game.battle.player.zone) ||
        ["LeftPocket","RightPocket","LeftRail","RightRail","XRailExit"].includes(Game.battle.cpu.zone);
