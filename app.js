...
<defs>
    <marker id="playerTrajectoryArrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
        <path d="M0,0 L7,3.5 L0,7 z" fill="#3ba8ff"/>
    </marker>
    <marker id="cpuTrajectoryArrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
        <path d="M0,0 L7,3.5 L0,7 z" fill="#ff4b4b"/>
    </marker>
</defs>
<line id="playerTrajectory" x1="500" y1="420" x2="500" y2="420" stroke="#3ba8ff" stroke-width="3" stroke-dasharray="5 7" stroke-linecap="round" marker-end="url(#playerTrajectoryArrow)" opacity="0"/>
<line id="cpuTrajectory" x1="500" y1="420" x2="500" y2="420" stroke="#ff4b4b" stroke-width="3" stroke-dasharray="5 7" stroke-linecap="round" marker-end="url(#cpuTrajectoryArrow)" opacity="0"/>
...
function getPredictedBattleZone(bey){
    const battle=Game.battle[bey];
    const opponent=Game.battle[bey==="player" ? "cpu" : "player"];
    const side=Game[bey];
    if(!battle || !opponent || !side?.blade) return null;
    if(battle.zone==="LeftRail" || battle.zone==="RightRail") return "XRailExit";
    if(battle.zone==="XRailExit") return "TopCenter";

    const neighbors=STADIUM_MAP[battle.zone]?.neighbors || [];
    if(!neighbors.length) return battle.zone;
    const combo=getBattleCombo(bey);
    ...
    for(const zone of neighbors){
        let score=0;
        if(zone===battle.previousZone) score-=8;
        if(centerZones.includes(zone)) score+=control*0.5;
        if(zone===opponent.zone) score+=aggression*0.8;
        if(STADIUM_MAP[zone]?.neighbors?.includes(opponent.zone)) score+=aggression*0.25+(stats.mobility||70)*0.15;
        if(dangerZones.includes(zone)) score-=type==="Attack" ? 12 : 30;
        if(battle.behavior==="chasing") score+=zone===opponent.zone?35:STADIUM_MAP[zone]?.neighbors?.includes(opponent.zone)?18:0;
        if(battle.behavior==="recovering" || battle.behavior==="retreating") score+=centerZones.includes(zone)?24:0;
        if(battle.behavior==="holding" && centerZones.includes(zone)) score+=16;
        if(type==="Attack" && (zone==="LeftRail" || zone==="RightRail")) score+=10;
        if(score>bestScore){ bestScore=score; best=zone; }
    }
    return best;
}

function setTrajectory(lineId,fromZone,toZone,visible=true){
    const line=document.getElementById(lineId);
    if(!line) return;
    const from=ZONE_POSITIONS[fromZone], to=ZONE_POSITIONS[toZone];
    if(!from || !to || fromZone===toZone){
    ...
    line.setAttribute("opacity",visible ? "0.72" : "0");
}
...
function renderBeys(){
    ...
    setTrajectory("playerTrajectory",Game.battle.player.zone,getPredictedBattleZone("player"),true);
    setTrajectory("cpuTrajectory",Game.battle.cpu.zone,getPredictedBattleZone("cpu"),true);
}
...
function pushBey(bey){
    ...
    const balance=combo?.stats?.balance||70;
    const stamina=combo?.stats?.stamina||70;
    const mobility=combo?.stats?.mobility||70;
    ...
    const railExitBonus=battle.railExitStrike ? (battle.railExitTargetZone==="Center" ? 18 : 8) : 0;

    // Knockback owns displacement. Attack only contributes indirectly through
    // the quality of the hit that created incoming force.
    const force=incoming*0.78 + attackerKnockback*0.30 + Math.max(0,battle.momentum)*0.12 + railExitBonus - defense*0.20 - balance*0.15 - mobility*0.07 - stamina*0.05;
    const normalized=Math.max(0,force);

    let pushes=1;
    if(normalized>42) pushes=2;
    if(normalized>72) pushes=3;
    if(normalized>105) pushes=4;
    if(normalized>138) pushes=5;
    if(defense>=90 && balance>=85 && normalized<105) pushes=Math.max(1,pushes-1);
    ...
    const preferDanger=normalized>48 && (attackerKnockback>76 || attackerAttack>82);
    ...
    for(let i=0;i<pushes;i++){
        ...
        const scored=destinations.map(zone=>{
            ...
            if(dangerZones.includes(zone)){
                score += preferDanger ? 26 : -12;
                if(zone==="XtremeZone") score += (normalized>88 && attackerKnockback>84) ? 20 : -18;
                if((zone==="LeftPocket" || zone==="RightPocket") && attackPower>82) score += 12;
            }
...
function resolveAutomaticSituation(){
    ...
    // Positive advantage must make a clean/heavy result MORE likely, not less.
    // The previous version subtracted advantage from the roll, which made a
    // stronger attacker drift toward near-misses/glancing hits and could
    // unnecessarily prolong battles.
    const adjustedRoll=Math.max(0,Math.min(100,Math.random()*100+advantage*0.55));
    ...
    if(railSide){
        ...
        if(adjustedRoll<8) event="nearMiss";
        else if(adjustedRoll<18) event="glancingHit";
        else if(adjustedRoll<32) event="pressure";
        else if(adjustedRoll<62) event="normalHit";
        ...
        if(Game[defender].currentMove==="Counter" && adjustedRoll<24){
            ...
            event="counterHit";
        }else if(adjustedRoll<6) event="nearMiss";
        else if(adjustedRoll<16) event="glancingHit";
        else if(adjustedRoll<30) event="pressure";
        else if(adjustedRoll<60) event="normalHit";
        else if(adjustedRoll<84) event="heavyHit";
        else {
            const power=(attackerCombo?.stats.attack||60)*0.45+
                (attackerCombo?.stats.knockback||60)*0.55;
...
function applyBattleEvent(event){
    ...
    const playerAttack=playerCombo?.stats?.attack||60;
    const cpuAttack=cpuCombo?.stats?.attack||60;
    const playerDefense=playerCombo?.stats?.defense||60;
    const cpuDefense=cpuCombo?.stats?.defense||60;
    const playerBalance=playerCombo?.stats?.balance||60;
    const cpuBalance=cpuCombo?.stats?.balance||60;

    // Attack determines hit quality. Knockback is reserved for displacement.
    const playerHitPower=playerAttack*0.70 + (playerCombo?.stats?.mobility||60)*0.12 + Math.max(0,player.momentum)*0.18;
    const cpuHitPower=cpuAttack*0.70 + (cpuCombo?.stats?.mobility||60)*0.12 + Math.max(0,cpu.momentum)*0.18;

    if(!winner){
        if(Math.abs(playerHitPower-cpuHitPower)<4) winner=Math.random()<0.5 ? "player" : "cpu";
        else winner=playerHitPower>cpuHitPower ? "player" : "cpu";
    }
    ...
    const defenderStamina=loserCombo?.stats?.stamina||60;
    const defenderMobility=loserCombo?.stats?.mobility||60;

    // Defense + Balance now provide substantial impact absorption. Stamina
    // helps prevent long-term collapse, and Mobility helps avoid displacement.
    const attackPower=attackStat*0.58 + (winnerState.speed||0)*0.10 + Math.max(0,winnerState.momentum)*0.18 + (winnerState.behavior==="chasing" ? 4 : 0);
    const defensePower=defenderDefense*0.42 + defenderBalance*0.32 + defenderStamina*0.14 + defenderMobility*0.12;
    const advantage=attackPower-defensePower;
    ...
    const advantageContribution=Math.max(-6,Math.min(18,advantage*0.17));
    const type=Game[loser]?.blade?.type || "Balance";
    const typeResistance=type==="Defense" ? 0.78 : type==="Stamina" ? 0.90 : type==="Balance" ? 0.94 : 1.0;

    let spinDamage=Math.round((3.5+advantageContribution)*strength*variance*typeResistance);
    let balanceDamage=Math.round((2.0+Math.max(-2,Math.min(10,advantage*0.10)))*strength*variance*typeResistance);

    // Never let a real hit become meaningless chip damage.
    spinDamage=Math.max(event==="glancingHit" ? 2 : 4,spinDamage);
    balanceDamage=Math.max(1,balanceDamage);
    if(event==="heavyHit") spinDamage=Math.max(10,spinDamage);
    if(event==="counterHit") spinDamage=Math.max(11,spinDamage);
    if(event==="railImpact") spinDamage=Math.max(14,spinDamage);
    if(event==="extremeImpact") spinDamage=Math.max(18,spinDamage);

    // Aggressive builds pay a small recoil cost on contact. This keeps Attack
    // powerful without making max ATK + max KNO a free universal strategy.
    const recoilBase={glancingHit:0.2,pressure:0.35,normalHit:0.55,counterHit:0.9,heavyHit:1.2,extremeImpact:1.8,railImpact:1.5}[event] || 0.4;
    const recoil=Math.max(0.25,recoilBase + Math.max(0,attackStat-82)*0.025 + Math.max(0,winnerState.speed-80)*0.01);

    loserState.spin=Math.max(0,loserState.spin-spinDamage);
    loserState.balance=Math.max(0,loserState.balance-balanceDamage);
    winnerState.spin=Math.max(0,winnerState.spin-recoil);

    winnerState.momentum=Math.min(100,winnerState.momentum+(event==="extremeImpact"?20:event==="heavyHit"?14:event==="railImpact"?17:event==="counterHit"?12:5));
    loserState.momentum=Math.max(-100,loserState.momentum-(event==="extremeImpact"?26:event==="heavyHit"?18:event==="railImpact"?20:event==="counterHit"?14:8));
    ...
    loserState.incomingForce=Math.max(0,
        knockbackStat*0.34 +
        Math.max(0,winnerState.momentum)*0.22 +
        (event==="extremeImpact"?30:event==="heavyHit"?20:event==="railImpact"?28:event==="counterHit"?13:0) +
        Math.max(0,advantage)*0.18
    );
    ...
    Game.battle.lastHitDamage={
        ...
        recoil:Number(recoil.toFixed(1)),
        beforeSpin,
        beforeBalance,
        ...
        loser,
        event,
        attackPower:Math.round(attackStat),
        knockbackPower:Math.round(knockbackStat),
        defenderDefense:Math.round(defenderDefense),
        defenderBalance:Math.round(defenderBalance),
        defenderStamina:Math.round(defenderStamina),
        momentum:Math.round(winnerState.momentum),
        momentumBonus:Math.round(Math.max(0,winnerState.momentum)),
        speed:Math.round(winnerState.speed),
        launchBonus:0,
        impactBase:Math.round(advantage),
        strength,
        enduranceFactor:typeResistance
    };

    if(event!=="pressure" || advantage>10) pushBey(loser);
...
HIT POWER · ${d.attackPower} ATK · ${d.knockbackPower} KNO
DEFENSE · ${d.defenderDefense} DEF · ${d.defenderBalance} BAL
MOMENTUM · +${Math.round(d.momentumBonus||0)} · ADVANTAGE ${advantage>=0?"+":""}${advantage}
DAMAGE · -${d.spin} STA · -${d.balance} BAL${d.recoil ? ` · RECOIL -${d.recoil} STA` : ""}`;
}
