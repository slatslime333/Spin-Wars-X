...
function showDynamicDecision(
        ...
        <section class="menu-card">
            ...
            <div class="battle-decision">
                ...
                ${choices.map(
                    ...
                    <button
                        type="button"
                        class="menu-btn gold decision-btn" data-decision-intent="${choice.intent}" onclick="chooseDynamicMove(this.dataset.decisionIntent);"
                    >
    ...
    // This legacy decision screen can also be used by the battle flow.
    document.querySelectorAll(".decision-btn[data-decision-intent]").forEach(btn=>{
        btn.onclick=(event)=>{
            event.preventDefault();
            event.stopPropagation();
            chooseDynamicMove(btn.dataset.decisionIntent);
        };
    });
...
function chooseDynamicMove(intent){
    ...
    const pending=Game.battle.pendingDecision || {choices:Game.battle.decisionChoices||[]};
    if(!Game.battle.decisionActive && !pending.choices.length) return;
    const choice=pending.choices.find(c=>c.intent===intent);
    if(!choice && !(Game.battle.decisionChoices||[]).some(c=>c.intent===intent)) return;
...
function renderBattleSequence(){
    ...
    if(live && Game.battle.decisionActive && Game.battle.pendingDecision){
        ...
        decisionPanel=`<div style="margin-top:8px;padding:10px;border-radius:10px;border:1px solid rgba(255,212,59,.45);background:rgba(255,212,59,.06);">
            ...
            ${d.choices.map((choice,i)=>{
                ...
                return `<button type="button" class="menu-btn gold decision-btn" style="margin-top:5px;padding:8px 10px;font-size:11px;text-align:left;cursor:pointer;position:relative;z-index:20;" data-decision-intent="${choice.intent}" onclick="chooseDynamicMove(this.dataset.decisionIntent);">
    ...
    // Bind tactical decisions directly after each battle-screen render.
    // This guarantees the freshly-created buttons call the actual resolver.
    document.querySelectorAll(".decision-btn[data-decision-intent]").forEach(btn=>{
        btn.onclick=(event)=>{
            event.preventDefault();
            event.stopPropagation();
...
window.addEventListener("DOMContentLoaded",()=>{
    ...
    // HARD FALLBACK: tactical buttons are recreated during battle renders.
    document.addEventListener("pointerdown",(event)=>{
        const button=event.target.closest?.(".decision-btn[data-decision-intent]");
        if(!button) return;
        event.preventDefault();
        event.stopPropagation();
        chooseDynamicMove(button.dataset.decisionIntent);
    },true);
});
