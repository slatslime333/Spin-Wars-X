...
function pushBey(bey){
    ...
    const railExitBonus=battle.railExitStrike
        ...
        : 0;
    const force=incoming*0.62+
        attackerKnockback*0.24+
        attackerAttack*0.14+
        Math.max(0,battle.momentum)*0.12+
        railExitBonus-
        defense*0.20-
        balance*0.12;

    let pushes=1;
    if(force>45) pushes=2;
    if(force>85) pushes=3;
    if(force>125) pushes=4;

    if(defense>88 && balance>75) pushes=Math.max(1,pushes-1);
    ...
    for(let i=0;i<pushes;i++){
        ...
        // safe recovery lanes.
        const dangerZones=["LeftPocket","RightPocket","LeftRail","RightRail","XRailExit"];
        const preferDanger=force>85 && (attackerKnockback>82 || attackerAttack>84);
        const scored=destinations.map(zone=>{
            let score=Math.random()*8;
            if(zone===opponent.zone) score-=8;
            if(dangerZones.includes(zone)) score+=preferDanger?18:-4;
...
function checkStadiumDanger(bey){
    ...
    if(zone==="LeftPocket" || zone==="RightPocket"){
        const combo=getBattleCombo(bey);
        const recovery=
            (combo?.stats?.defense||70)*0.28+
            battle.balance*0.42+
            (combo?.stats?.stamina||70)*0.18+
            (combo?.stats?.mobility||70)*0.12;
        const roll=Math.random()*100;
        const safeChance=Math.max(8,Math.min(92,recovery));
        if(roll>safeChance){
    ...
    // XTREME: rare, but reachable. A high-speed rail route can finish even
    // when the attacker was not the original aggressor (deflection included).
    if(zone==="XRailExit"){
        const combo=getBattleCombo(bey);
        const power=
            (combo?.stats?.attack||60)*0.45+
            (combo?.stats?.knockback||60)*0.55;
        const momentum=Math.abs(battle.momentum)+Math.abs(battle.railSpeed||0)*0.35;
        const threshold=62;
        if(momentum>=threshold){
            const quality=Game[bey].launch.quality;
            const qualityBonus={Perfect:10,Good:6,Okay:0,Bad:-5,Horrible:-10}[quality]||0;
            const chance=Math.max(20,Math.min(88,28+(momentum-62)*1.05+(power-70)*0.18+qualityBonus));
            if(momentum>=82 || Math.random()*100<chance){
...
function generateDynamicDecision(){
    ...
    const previous=Game.battle.lastHitDamage;
    const previousStory=getRecentBattleStory();
    const playerCondition=player.spin<45 ? `${playerName}'s spin is starting to fall away.` : player.balance<45 ? `${playerName}'s balance is becoming the immediate concern.` : `${playerName} still has enough spin and balance to make something happen.`;
    const cpuCondition=cpu.spin<45 ? `${cpuName} is also beginning to feel the loss of spin.` : cpu.balance<45 ? `${cpuName} is fighting to stay stable.` : `${cpuName} still has a stable platform to work from.`;
    let scenario="";

    if(["LeftPocket","RightPocket"].includes(playerZone)){
        scenario=`${playerName} has been driven into ${formatBattleZone(playerZone)}. ${cpuName} is ${getStateDetail("cpu")}, and the escape lane is closing. ${previousStory} ${playerCondition} This is a survival decision: get back into the bowl, or try to turn ${cpuName}'s pressure against it.`;
        choices.push({name:"Fight out toward safety",intent:"escape"},{name:"Stabilize and rebuild",intent:"stabilize"},{name:"Turn the pressure into a counter",intent:"counter"});
    }else if(player.rail || playerZone==="XRailExit" || cpu.rail || cpuZone==="XRailExit"){
        if(player.rail || playerZone==="XRailExit"){
            scenario=`${playerName} has the rail. The ride has already built speed, and ${cpuName} is sitting at ${formatBattleZone(cpuZone)}. ${previousStory} The question is whether ${playerName} can turn that speed into an exit strike or burn the run trying to force it. ${playerCondition}`;
            choices.push({name:"Commit to the X Rail strike",intent:"attack"},{name:"Control the exit and stay safe",intent:"hold"},{name:"Break away before the exit",intent:"evade"});
        }else{
            scenario=`${cpuName} is on the X Rail and building speed toward the X Exit. ${playerName} is at ${formatBattleZone(playerZone)}. ${previousStory} ${cpuCondition} The danger is not just the rail itself — it is where that speed will carry ${cpuName} when it leaves the track.`;
            choices.push({name:"Brace and absorb the exit",intent:"brace"},{name:"Move off the exit line",intent:"evade"},{name:"Meet the rail with a counter",intent:"counter"});
        }
    }else if(["normalHit","glancingHit","pressure","heavyHit","extremeImpact","counterHit"].includes(event)){
        const hitStory=previous ? `${winnerNameForStory()} just changed the exchange, leaving the defender with less spin and a less comfortable line.` : "";
        scenario=`The last exchange is still shaping the battle. ${previousStory} ${hitStory} ${playerName} is ${getStateDetail("player")}, while ${cpuName} is ${getStateDetail("cpu")}. ${playerCondition} ${cpuCondition} You are not choosing from a blank slate — your move decides whether the next collision happens on your terms.`;
        choices.push({name:"Brace and make them pay for the next hit",intent:"brace"},{name:"Set the trap for a counter",intent:"counter"},{name:"Slip the line and attack the next opening",intent:"evade"});
    }else if(STADIUM_MAP[playerZone]?.neighbors?.includes(cpuZone) || STADIUM_MAP[cpuZone]?.neighbors?.includes(playerZone)){
        scenario=`The lanes are closing. ${playerName} is at ${formatBattleZone(playerZone)} and ${cpuName} is at ${formatBattleZone(cpuZone)}. ${previousStory} ${playerName} is ${getStateDetail("player")}; ${cpuName} is ${getStateDetail("cpu")}. ${playerCondition} ${cpuCondition} One clean angle here could decide who owns the center of the next exchange.`;
        choices.push({name:"Meet them before they settle",intent:"attack"},{name:"Invite the attack and counter",intent:"counter"},{name:"Change lanes and make them chase",intent:"evade"});
    }else{
        scenario=`The battle has opened up. ${playerName} is working from ${formatBattleZone(playerZone)} while ${cpuName} is at ${formatBattleZone(cpuZone)}. ${previousStory} ${playerName} is ${getStateDetail("player")}; ${cpuName} is ${getStateDetail("cpu")}. This is the quiet part of the fight where small positioning choices become the next big collision.`;
        choices.push({name:pCombo?.stats?.mobility>82?"Hunt the opponent":"Take the center",intent:pCombo?.stats?.mobility>82?"chase":"center"},{name:"Hold the line and conserve spin",intent:"hold"},{name:"Change the angle and draw them in",intent:"evade"});
    }
...
function executeXRailDecision(bey){
    ...
    showIntentResult("Commit to X Rail + Attack","CPU response","resolved",
        `${Game.player.blade.name} commits to the X Rail — the run carries into the exit strike.`);
}
...
function resolveAutomaticSituation(){
    ...
    const pPower=(pCombo?.stats.attack||60)*0.45+
        (pCombo?.stats.knockback||60)*0.35+
        (pCombo?.stats.mobility||60)*0.20+
        player.speed*0.08+
        player.momentum*0.28;
    const cPower=(cCombo?.stats.attack||60)*0.45+
        (cCombo?.stats.knockback||60)*0.35+
        (cCombo?.stats.mobility||60)*0.20+
        cpu.speed*0.08+
        cpu.momentum*0.28;
    ...
    const advantage=
        ((attackerCombo?.stats.attack||60)-(defenderCombo?.stats.defense||60))*0.28+
        ((attackerCombo?.stats.knockback||60)-(defenderCombo?.stats.balance||60))*0.34+
        ((attackerCombo?.stats.mobility||60)-(defenderCombo?.stats.mobility||60))*0.10+
        (Math.abs(Game.battle[attacker].momentum)-Math.abs(Game.battle[defender].momentum))*0.18;

    const adjustedRoll=Math.max(0,Math.min(100,Math.random()*100-advantage*0.42));
    let event="separation";

    if(railSide){
        const railPower=(attackerCombo?.stats.attack||60)*0.45+(attackerCombo?.stats.knockback||60)*0.55;
        if(adjustedRoll<34) event="railDash";
        else if(railPower>86 && adjustedRoll>78) event="railImpact";
        else event="railDash";
    }else if(situation==="separated"){
        if(adjustedRoll<36) event="reposition";
        else if(adjustedRoll<55) event="nearMiss";
        else if(adjustedRoll<73) event="separation";
        else event="pressure";
    }else if(near){
        if(adjustedRoll<13) event="nearMiss";
        else if(adjustedRoll<29) event="glancingHit";
        else if(adjustedRoll<49) event="pressure";
        else if(adjustedRoll<79) event="normalHit";
        else event="heavyHit";
    }else if(close){
        if(Game[defender].currentMove==="Counter" && adjustedRoll<30){
            Game.battle.forcedWinner=defender;
            event="counterHit";
        }else if(adjustedRoll<9) event="nearMiss";
        else if(adjustedRoll<25) event="glancingHit";
        else if(adjustedRoll<45) event="pressure";
        else if(adjustedRoll<74) event="normalHit";
        else if(adjustedRoll<94) event="heavyHit";
        else {
            // Extreme impact is a rare event. Strong attack/knockback builds
            // are much more likely to create it.
            const power=(attackerCombo?.stats.attack||60)*0.45+
                (attackerCombo?.stats.knockback||60)*0.55;
            event=power>=86 ? "extremeImpact" : "heavyHit";
        }
    }
...
function applyBattleEvent(event){
    ...
    const playerPower=(playerCombo?.stats.attack||60)*0.48+
        (playerCombo?.stats.knockback||60)*0.52+
        player.speed*0.12+player.momentum*0.30+(player.attackBonus||0);
    const cpuPower=(cpuCombo?.stats.attack||60)*0.48+
        (cpuCombo?.stats.knockback||60)*0.52+
        cpu.speed*0.12+cpu.momentum*0.30+(cpu.attackBonus||0);

    if(!winner) winner=playerPower>=cpuPower ? "player" : "cpu";
    ...
    let strength=1;
    if(event==="glancingHit") strength=0.70;
    if(event==="pressure") strength=0.90;
    if(event==="normalHit") strength=1.05;
    if(event==="counterHit") strength=1.35;
    if(event==="heavyHit") strength=2.05;
    if(event==="extremeImpact") strength=2.70;
    if(event==="railImpact") strength=2.35;

    const launchBonus=Game[winner].launch?.attackBonus||0;
    const attackPower=winnerCombo?.stats?.attack||60;
    const knockbackPower=winnerCombo?.stats?.knockback||60;
    const defenderDefense=loserCombo?.stats?.defense||60;
    const defenderBalance=loserCombo?.stats?.balance||60;

    const impactBase=Math.max(0,
        attackPower*0.46+
        knockbackPower*0.54+
        winnerState.momentum*0.38+
        winnerState.speed*0.12+
        launchBonus+
        (winnerState.railExitStrike ? (winnerState.railExitTargetZone==="Center" ? 24 : 12) : 0)-
        (defenderDefense*0.52+defenderBalance*0.48)
    );

    // Stamina matters, but only as a modest endurance modifier.
    const enduranceFactor=Math.max(0.88,Math.min(1.12,88/(loserCombo?.stats?.stamina||80)));
    const spinDamage=Math.max(1,Math.round(
        (4.5+impactBase*0.20)*strength*enduranceFactor+
        Math.random()*2+
        Math.max(0,winnerState.momentum)*0.05
    ));
    const balanceDamage=Math.max(1,Math.round(
        (2.5+impactBase*0.11)*strength+
        Math.random()*2.5
    ));
    ...
    Game.battle.lastHitDamage={
        ...
        launchBonus:Math.round(launchBonus),
        impactBase,
        strength,
        enduranceFactor
    };
    ...
    if(event!=="pressure" || impactBase>10){
        pushBey(loser);
    }
...
function getImpactCalculationText(){
    ...
    const baseCalc=
        `(${d.attackPower} ATK × 0.46) + (${d.knockbackPower} KB × 0.54) + `+
        `(${d.momentum} MOM × 0.38) + (${d.speed} SPD × 0.12) + ${d.launchBonus} LAUNCH − `+
        `(${d.defenderDefense} DEF × 0.52 + ${d.defenderBalance} BAL × 0.48)`;

    return `IMPACT · ${winnerName} → ${loserName}
CALC · ${baseCalc} = ${d.impactBase.toFixed(1)} base
MULTIPLIER · ${d.event.toUpperCase()} × ${d.strength.toFixed(2)} · STA factor ${d.enduranceFactor.toFixed(2)}
...
function generateEventCommentary(event){
    ...
    if(event==="extremeImpact"){
        const lines=[
            `BOOM! ${winnerName} catches ${loserName} at speed and the impact changes the geometry of the stadium. ${loserName} is no longer choosing where this battle goes.`,
            `${winnerName} hits with everything the opening has built — speed, momentum and contact all arrive together. ${loserName} gets launched into a dangerous line.`,
            `That is the collision everyone was waiting for. ${winnerName} turns the approach into a violent impact and sends ${loserName} scrambling for survival.`
        ];
        return lines[Math.floor(Math.random()*lines.length)];
    }
...
function decideNextBattleStep(){
    ...
    // No artificial round/turn limit. A round continues until a real finish
...
function checkBattleFinish(){
    ...
    if(player.balance<=0){
        Game.battle.finish="Over Finish"; Game.battle.finishPoints=2; Game.battle.winner="cpu";
        endBattleRound(); return true;
    }
    if(cpu.balance<=0){
        Game.battle.finish="Over Finish"; Game.battle.finishPoints=2; Game.battle.winner="player";
        endBattleRound(); return true;
    }
    return false;
}
...
🎙 ${getLaunchStory("player")}
${getLaunchImpactSummary("player")}

🎙 ${getLaunchStory("cpu")}
${getLaunchImpactSummary("cpu")}

The launch quality is already shaping the fight: speed creates opportunities, but speed also costs spin. Stamina and launch efficiency decide how long each Bey can keep moving at that pace.
