...
function showIntentResult(intent,cpuIntent,result,text){
    ...
    const impactText=getImpactReasonText();
...
function getImpactReasonText(){
    const d=Game.battle.lastHitDamage;
    if(!d || !d.winner || !d.event) return "";
    const winnerName=d.winner==="player" ? Game.player.blade.name : Game.cpu.blade.name;
    const loserName=d.loser==="player" ? Game.player.blade.name : Game.cpu.blade.name;
    const attack=(d.attackPower||60)*0.45+(d.knockbackPower||60)*0.55;
    const defense=(d.defenderDefense||60)*0.5+(d.defenderBalance||60)*0.5;
    const margin=attack-defense;
    if(margin>=20) return `WHY IT WORKED · ${winnerName}'s attack/knockback advantage overwhelmed ${loserName}'s stability.`;
    if(margin>=5) return `WHY IT WORKED · ${winnerName} won the contact by a narrow impact margin.`;
    return `WHY IT WORKED · ${winnerName} found the better angle and converted the exchange despite ${loserName}'s resistance.`;
}
...
function renderBattleSequence(){
    ...
    if(live && Game.battle.decisionActive && Game.battle.pendingDecision){
        ...
        decisionPanel=`<div style="margin-top:8px;padding:10px;border-radius:10px;border:1px solid rgba(255,212,59,.45);background:rgba(255,212,59,.06);">
            ...
            ${d.choices.map((choice,i)=>{
                ...
                return `<button class="menu-btn gold decision-btn" style="margin-top:5px;padding:8px 10px;font-size:11px;text-align:left;" data-decision-intent="${choice.intent}">
    ...
    document.querySelectorAll(".decision-btn[data-decision-intent]").forEach(btn=>{
        btn.onclick=()=>chooseDynamicMove(btn.dataset.decisionIntent);
    });
