...
<!-- X EXIT / CENTER DROP -->
<path class="rail-track rail-exit-line" d="M500 92 C500 160 500 225 500 300 C500 350 500 390 500 430"/>
...
function applyNaturalSpinCost(bey){
    ...
    const activeCost=0.90 + bit.spinDrain*(0.42+movementRatio*0.78+momentum/1800);
    const retention=(combo.stats.stamina||70)/100;
    const staminaFactor=1.16-retention*0.36;
    const cost=Math.max(0.18,activeCost*staminaFactor*qualityFactor);
...
function resolveAutomaticSituation(){
    ...
    if(aState.zone==='XRailExit' || aState.railExitStrike){
        ...
        if(roll<34) event='reposition';
        else if(roll<58) event='nearMiss';
        else if(roll<78) event='separation';
...
function getImpactCalculationText(){
    ...
    return `CALC · ${winnerName}\nPOWER ${Math.round(d.attackPower)} · DEF ${Math.round(d.defenderDefense)} · MOM ${Math.round(d.momentumBonus||0)} · ADV ${advantage>=0?'+':''}${advantage}\nDAMAGE · -${Math.round(d.spin)} STA · -${Math.round(d.balance)} BAL`;
}
...
function resolveAutomaticEvent(event){
    ...
    const pathText=movement ? `PATH · YOU ${movement.player.from} → ${movement.player.to} · CPU ${movement.cpu.from} → ${movement.cpu.to}` : "";
    if(pathText && !/PATH ·/.test(eventText)) eventText+=`\n\n${pathText}`;
...
function getLaunchPath(blader){
    ...
    const spin=Game[blader].blade?.spin || 'Right';
    const naturalSide=spin==='Right'?'Left':'Right';
    const start=side==='Left'?'LeftMid':'RightMid';
    const sameSide=side===naturalSide;
    const bit=getBitPhysics(blader);
    const committed=launch.xRailCommitted;

    if(launch.technique==='X-Rail Dash' || launch.technique==='X-Rail'){
        if(!committed) return [start, side==='Left'?'TopLeft':'TopRight','Center'];
        const railStart=side==='Left'?'LeftRail':'RightRail';
        const upper=side==='Left'?['RailLeftTop','XRailExit']:['RailRightTop','XRailExit'];
        return [start,railStart,...upper,'Center'];
    }

    if(launch.technique==='Reverse X-Dash'){
        if(bit.xRailAffinity<30 && launch.quality!=='Perfect') return [start,side==='Left'?'BottomLeft':'BottomRight','Center'];
        const first=side==='Left'?'LeftRail':'RightRail';
        const lower=side==='Left'?['RailLeftBottom','RailBottom','RailRightBottom','RightRail','RailRightTop','XRailExit']:
            ['RailRightBottom','RailBottom','RailLeftBottom','LeftRail','RailLeftTop','XRailExit'];
        return [start,first,...lower,'Center'];
    }

    if(launch.technique==='Direct Clash'){
        return [start,side==='Left'?'LeftMid':'RightMid','Center'];
    }

    if(launch.technique==='Drop Launch'){
        return side==='Left'
            ? [start,'BottomLeft','BottomCenter']
            : [start,'BottomRight','BottomCenter'];
    }

    if(launch.technique==='Wide Circle'){
        return side==='Left'
            ? [start,'TopLeft','TopCenter','Center']
            : [start,'TopRight','TopCenter','Center'];
    }

    // Center launch: bit determines whether the opening line is aggressive or stable.
