...
function getLaunchQualityText(quality){
    return {
        Perfect:"maximum control and opening momentum",
        Good:"strong control and momentum",
        Okay:"clean but neutral",
        Bad:"reduced control and momentum",
        Horrible:"poor control and a weak opening"
    }[quality] || "neutral";
}
...
function generateDynamicDecision(){
    ...
    if(["LeftPocket","RightPocket"].includes(playerZone)){
        scenario=`DANGER! ${playerName} is in ${formatBattleZone(playerZone)}. ${cpuName} is pressing.`;
        choices.push({name:"Escape",intent:"escape"},{name:"Stabilize",intent:"stabilize"},{name:"Counter",intent:"counter"});
    }else if(player.balance<40 || player.spin<40){
        scenario=`RECOVERY. ${playerName} is ${playerState.toLowerCase()} at ${Math.round(player.spin)} STA / ${Math.round(player.balance)} BAL.`;
        choices.push({name:"Stabilize",intent:"stabilize"},{name:"Evade",intent:"evade"},{name:"Attack",intent:"attack"});
    }else if(player.rail || playerZone==="XRailExit" || cpu.rail || cpuZone==="XRailExit"){
        if(player.rail || playerZone==="XRailExit"){
            scenario=`X RAIL. ${playerName} has the run. ${cpuName} is ${formatBattleZone(cpuZone)} — commit for a high-speed exit strike or play it safe.`;
            choices.push({name:"Commit to X Rail + Attack",intent:"attack"},{name:"Control the exit",intent:"hold"},{name:"Break off",intent:"evade"});
        }else{
            scenario=`RAIL THREAT. ${cpuName} is building speed. Get clear, brace, or meet it.`;
            choices.push({name:"Brace",intent:"brace"},{name:"Evade",intent:"evade"},{name:"Counter",intent:"counter"});
        }
    }else if(["normalHit","glancingHit","pressure","heavyHit","extremeImpact","counterHit"].includes(event)){
        scenario=`CLASH. ${playerName} is ${playerState.toLowerCase()}; ${cpuName} is ${cpuState.toLowerCase()}.`;
        choices.push({name:"Brace",intent:"brace"},{name:"Counter",intent:"counter"},{name:"Evade",intent:"evade"});
    }else{
        scenario=`POSITION. ${playerName}: ${formatBattleZone(playerZone)} · ${cpuName}: ${formatBattleZone(cpuZone)}.`;
        choices.push(
            {name:pCombo?.stats?.mobility>82?"Chase":"Take center",intent:pCombo?.stats?.mobility>82?"chase":"center"},
            {name:"Hold line",intent:"hold"},
            {name:"Change path",intent:"evade"}
        );
    }
...
function generateEventCommentary(event){
    ...
    if(event==="nearMiss") return `MISS! ${pName} and ${cName} flash past.`;
    if(event==="reposition") return `RESET. Both break off and hunt for a better line.`;
    if(event==="railDash") return `X RAIL! ${p.rail?pName:cName} is flying toward the exit.`;
    if(event==="separation") return `SPACE. Neither wants the bad angle.`;
    if(event==="pocketEscape") return `ESCAPE! A recovery saves the round.`;
    if(!winner || !damage) return `They collide, but neither takes control.`;
    const winnerName=winner==="player"?pName:cName;
    const loserName=winner==="player"?cName:pName;
    const combo=getBattleCombo(winner);
    const power=Math.round((combo?.stats.attack||60)*0.45+(combo?.stats.knockback||60)*0.55);
    const flavor=power>=90 ? "Pure power." : power>=80 ? "Great angle." : "Momentum decides it.";
    if(event==="glancingHit") return `CLIP! ${winnerName} catches ${loserName}. -${damage.spin} STA.`;
    if(event==="pressure") return `PRESSURE! ${winnerName} keeps ${loserName} moving. -${damage.spin} STA.`;
    if(event==="counterHit") return `COUNTER! ${winnerName} sends it back. ${flavor}`;
    if(event==="railImpact") return `RAIL STRIKE! ${winnerName} blasts out of the exit. -${damage.spin} STA.`;
    if(event==="normalHit") return `CLEAN HIT! ${winnerName} connects. -${damage.spin} STA.`;
    if(event==="heavyHit") return `HEAVY! ${winnerName} breaks the line. -${damage.spin} STA, -${damage.balance} BAL.`;
    if(event==="extremeImpact") return `BOOM! ${winnerName} catches ${loserName} at full speed.`;
    return `${winnerName} wins the exchange. ${flavor}`;
}
...
function startBattleRound(){
    ...
    p.speed=clampBattleValue(
        (pCombo?.stats?.mobility ?? 70)+(Game.player.launch.movementBonus||0)*0.6
    );
    ...
    c.speed=clampBattleValue(
        (cCombo?.stats?.mobility ?? 70)+(Game.cpu.launch.movementBonus||0)*0.6
    );
...
${Game.player.blade.name} · ${Game.player.launch.technique} · ${Game.player.launch.quality}
${Game.cpu.blade.name} · ${Game.cpu.launch.technique} · ${Game.cpu.launch.quality}

LAUNCH EDGE
YOU: ${Math.round(p.spin)} stamina · ${Math.round(p.balance)} balance
CPU: ${Math.round(c.spin)} stamina · ${Math.round(c.balance)} balance`;
