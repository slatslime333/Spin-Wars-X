...
function generateDynamicDecision(){
    ...
    if(["LeftPocket","RightPocket"].includes(playerZone)){
        ...
        scenario=`X RAIL. Speed is building. Do you commit, control the exit, or get clear?`;
        choices.push({name:"Commit to rail",intent:"attack"},{name:"Control exit",intent:"hold"},{name:"Evade",intent:"evade"});
...
function simulateBattleRound(){
    ...
    if(Game.battle.turn>Game.battle.maxTurns){
        const playerSpin=Game.battle.player.spin;
        const cpuSpin=Game.battle.cpu.spin;
        const winner=playerSpin>=cpuSpin ? "player" : "cpu";
        const loser=winner==="player" ? "cpu" : "player";
        Game.battle[loser].spin=0;
        Game.battle.finish="Spin Finish";
        Game.battle.finishPoints=1;
        Game.battle.winner=winner;
        Game.battle.finishCause="Endurance";
        endBattleRound();
        return;
    }
...
function applyBattleEvent(event){
    ...
    // Use the attacker’s Knockback to drive actual displacement.
...
function generateEventCommentary(event){
    ...
    if(event==="nearMiss") return `MISS! ${pName} and ${cName} cross cleanly. ${pName} ${formatBattleZone(p.zone)} · ${cName} ${formatBattleZone(c.zone)}.`;
    if(event==="reposition") return `RESET. Both Beys change lanes — neither wants to commit from a bad angle.`;
    if(event==="railDash") return `X RAIL! ${p.rail?pName:cName} is building speed for the exit.`;
    if(event==="separation") return `SPACE. They stay apart and hunt for the next opening.`;
    if(event==="pocketEscape") return `POCKET ESCAPE! A recovery keeps the round alive.`;

    if(!winner || !damage) return `${pName} and ${cName} clash without a clean advantage.`;

    const winnerName=winner==="player"?pName:cName;
    const loserName=winner==="player"?cName:pName;
    const loserState=Game.battle[winner==="player"?"cpu":"player"];
    const loserCombo=getBattleCombo(winner==="player"?"cpu":"player");
    const power=winner==="player"
        ? getBattleCombo("player")
        : getBattleCombo("cpu");
    const attack=Math.round((power?.stats.attack||60)*0.45+(power?.stats.knockback||60)*0.55);
    const reason=attack>=88 ? "Pure power drives it through." :
        attack>=78 ? "The contact angle favors the attacker." :
        "Timing and momentum decide it.";

    if(event==="glancingHit") return `GLANCING HIT — ${winnerName} clips ${loserName}. ${reason} -${damage.spin} STA.`;
    if(event==="pressure") return `PRESSURE — ${winnerName} keeps ${loserName} moving. -${damage.spin} STA, -${damage.balance} BAL.`;
    if(event==="counterHit") return `COUNTER! ${winnerName} turns the attack back. ${reason}`;
    if(event==="railImpact") return `X RAIL HIT! ${winnerName} converts rail speed into impact. -${damage.spin} STA.`;
    if(event==="normalHit") return `HIT! ${winnerName} lands clean. -${damage.spin} STA, -${damage.balance} BAL.`;
    if(event==="heavyHit") return `HEAVY! ${winnerName} breaks ${loserName}'s line. -${damage.spin} STA, -${damage.balance} BAL.`;
    if(event==="extremeImpact") return `EXTREME IMPACT! ${winnerName} catches ${loserName} at speed. -${damage.spin} STA.`;
    return `${winnerName} wins the exchange. ${reason}`;
}
