...
function renderBattleStatus(key){
    ...
    return `
        <div style="padding:14px;border-radius:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.10);">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:center;"><strong>${name}</strong><strong>${state}</strong></div>
            <div style="margin-top:9px;font-size:13px;display:flex;justify-content:space-between;"><span>STAMINA</span><span>${stamina}%</span></div>
            <div style="height:8px;background:rgba(255,255,255,.10);border-radius:8px;overflow:hidden;margin-top:4px;"><div style="width:${stamina}%;height:100%;background:#e7e7e7;"></div></div>
            <div style="margin-top:8px;font-size:12px;display:flex;justify-content:space-between;opacity:.82;"><span>Balance ${balance}%</span><span>Momentum ${momentum}%</span></div>
            <div style="margin-top:6px;font-size:12px;opacity:.72;">${getStateDetail(key)} · ${b.zone}</div>
        </div>`;
}
function renderBattleStatusPanel(){
    return `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0;">${renderBattleStatus("player")}${renderBattleStatus("cpu")}</div>`;
}
...
function simulateBattleMovement(bey){
    ...
    if(isXRailZone(battle.zone)){ handleXRailMovement(bey); return; }
    updateBattleBehavior(bey);
    if(battle.zone==="XExit"){
        const railResult=resolveXRailExit(bey);
        Game.battle.railEvent=railResult;
        if(!railResult.resolved){
            const exitNeighbors=STADIUM_MAP[battle.zone]?.neighbors || [];
            if(exitNeighbors.length) moveBey(bey,exitNeighbors[0]);
        }
        return;
    }
    ...
    const railZones=["XRailTop","XRailRight","XRailBottom","XRailLeft"];
    const dangerZones=["LeftPocket","RightPocket"];
    let bestZone=neighbors[0];
    ...
    for(const zone of neighbors){
        ...
        if(danger){ score-=80; if(spin<30 || balance<30) score+=risk*0.25; }
        if(center) score+=control*0.45;
        if(zone===battle.previousZone) score-=8;
        if(momentum>15 && approachesOpponent) score+=momentum*0.45;
        if(momentum<-15 && !approachesOpponent) score+=Math.abs(momentum)*0.25;
        if(type==="Attack"){
            if(touchesOpponent) score+=aggression*0.95;
            if(approachesOpponent) score+=aggression*0.55;
            if(rail) score+=risk*0.60;
            if(center) score+=control*0.15;
        }else if(type==="Defense"){
            if(center) score+=control*0.75;
            if(approachesOpponent) score+=aggression*0.15;
            if(touchesOpponent) score+=aggression*0.25;
            if(rail) score-=risk*0.45;
        }else if(type==="Stamina"){
            if(center) score+=control*0.95;
            if(approachesOpponent) score-=control*0.45;
            if(touchesOpponent) score-=control*0.75;
            if(rail) score-=risk*0.70;
        }else{
            if(center) score+=control*0.55;
            if(approachesOpponent) score+=aggression*0.35;
            if(touchesOpponent) score+=aggression*0.55;
            if(rail) score+=risk*0.25;
        }
        if(battle.behavior==="chasing"){
            if(touchesOpponent) score+=55;
            if(approachesOpponent) score+=30;
        }else if(battle.behavior==="retreating" || battle.behavior==="recovering"){
            if(!approachesOpponent) score+=25;
            if(center) score+=20;
        }else if(battle.behavior==="circling"){
            if(!danger && zone!==battle.previousZone) score+=18;
        }
        score+=Math.random()*4;
...
function resolveAutomaticSituation(){
    ...
    const pAttack=(pCombo?.stats.attack || 60)+(pCombo?.stats.knockback || 60)*0.45+player.speed*0.25+player.momentum*0.20;
    const cAttack=(cCombo?.stats.attack || 60)+(cCombo?.stats.knockback || 60)*0.45+cpu.speed*0.25+cpu.momentum*0.20;
    const pDefense=(pCombo?.stats.defense || 60)+(pCombo?.stats.balance || 60)*0.45;
    const cDefense=(cCombo?.stats.defense || 60)+(cCombo?.stats.balance || 60)*0.45;
    const pMob=pCombo?.stats.mobility || 60;
    const cMob=cCombo?.stats.mobility || 60;
    const pType=Game.player.blade?.type || "Balance";
    const cType=Game.cpu.blade?.type || "Balance";
    let event="separation";
    let engagement=situation==="clash" ? 100 : situation==="crossing" ? 82 : situation==="approach" ? 62 : 25;
    const attackBias=Math.max(pAttack,cAttack);
    const speedBias=Math.max(player.speed+pMob*0.2,cpu.speed+cMob*0.2);
    if(situation==="separated"){
        const pressure=attackBias+speedBias;
        event=pressure>155 && Math.random()*100<28 ? "passBy" : "separation";
    }else{
        const pHitChance=Math.max(10,Math.min(90,50+(pAttack-cAttack)*0.28+(pMob-cMob)*0.12));
        const winnerLikely=pHitChance>=50 ? "player" : "cpu";
        const winnerType=winnerLikely==="player" ? pType : cType;
        const winnerAttack=winnerLikely==="player" ? pAttack : cAttack;
        const loserDefense=winnerLikely==="player" ? cDefense : pDefense;
        const pressure=Math.max(0,winnerAttack-loserDefense);
        let missChance=10;
        if(winnerType==="Stamina") missChance+=10;
        if(winnerType==="Defense") missChance+=6;
        if(situation==="approach") missChance+=8;
        if(Math.max(pMob,cMob)>90) missChance-=4;
        missChance=Math.max(4,Math.min(28,missChance));
        if(Math.random()*100<missChance){
            event="passBy";
        }else{
            const heavyChance=Math.max(14,Math.min(58,22+pressure*0.35+(engagement-50)*0.20));
            const extremeChance=Math.max(2,Math.min(16,3+pressure*0.12+Math.max(player.momentum,cpu.momentum)*0.08));
            const impactRoll=Math.random()*100;
            if(impactRoll<extremeChance) event="extremeImpact";
            else if(impactRoll<extremeChance+heavyChance) event="heavyHit";
            else event="normalHit";
        }
    }
    Game.battle.lastEvent=event;
...
function applyBattleEvent(event){
    ...
    const playerPower=
        ...
        player.speed*0.20+
        player.momentum*0.18+
        (player.attackBonus||0)+
        (Game.player.blade?.personality?.aggression||50)*0.08+
        (Math.random()*12-6);
    ...
    const cpuPower=
        ...
        cpu.momentum*0.18+
        (cpu.attackBonus||0)+
        (Game.cpu.blade?.personality?.aggression||50)*0.08+
        (Math.random()*12-6);
   ...
   if(Game.battle.forcedWinner==="player"){
    ...
    const loserStateBefore=getBeyState(loser);
    const loserSpinBefore=Game.battle[loser].spin;
    const loserBalanceBefore=Game.battle[loser].balance;
...
if(
    ...
    Game.battle.lastHitDamage={
        ...
        beforeSpin:loserSpinBefore,
        beforeBalance:loserBalanceBefore,
        afterSpin:Game.battle[loser].spin,
        afterBalance:Game.battle[loser].balance,
        beforeState:loserStateBefore,
...
function generateEventCommentary(event){
    ...
    if(event==="passBy"){
        const attacker=player.momentum+player.speed>=cpu.momentum+cpu.speed ? playerName : cpuName;
        const defender=attacker===playerName ? cpuName : playerName;
        return `${attacker} creates the opening, but ${defender} avoids clean contact. Both Beys reset their lines.`;
    }
    if(event==="separation"){
        return `${playerName} and ${cpuName} separate and search for a better angle. No clean attack develops.`;
    }
    if(!winner || !damage){
        return `${playerName} and ${cpuName} make contact, but neither gains a clear advantage.`;
    }

    const winnerName=winner==="player" ? playerName : cpuName;
    const loserName=winner==="player" ? cpuName : playerName;
    const winnerCombo=winner==="player"
    ...
    const loserCombo=winner==="player"
        ? calculateComboStats(Game.cpu.blade,Game.cpu.ratchet,Game.cpu.bit)
        : calculateComboStats(Game.player.blade,Game.player.ratchet,Game.player.bit);
    const attackPower=winnerCombo.stats.attack+winnerCombo.stats.knockback;
    const defensePower=loserCombo.stats.defense+loserCombo.stats.balance;
    const difference=attackPower-defensePower;
    let reason;
    if(difference>=25) reason=`${winnerName}'s power overwhelms ${loserName}'s defense.`;
    else if(difference>=0) reason=`${winnerName} gets the better of the exchange, but ${loserName} absorbs part of the force.`;
    else reason=`${loserName} resists well, but ${winnerName} finds the better contact.`;

    if(event==="normalHit"){
        return `${winnerName} lands a clean hit on ${loserName}. ${reason} ${loserName}: -${damage.spin} stamina, -${damage.balance} balance. State ${damage.beforeState} → ${damage.afterState}.`;
    }
    if(event==="heavyHit"){
        return `HEAVY IMPACT! ${winnerName} breaks ${loserName}'s line. ${reason} ${loserName}: -${damage.spin} stamina, -${damage.balance} balance. State ${damage.beforeState} → ${damage.afterState}.`;
    }
    if(event==="extremeImpact"){
        return `EXTREME IMPACT! ${winnerName} connects with maximum force. ${reason} ${loserName}: -${damage.spin} stamina, -${damage.balance} balance. State ${damage.beforeState} → ${damage.afterState}.`;
    }
    return `${winnerName} gains the exchange.`;
}
...
function startBattleRound(){
    ...
    Game.battle.player.behavior="circling";
    Game.battle.player.behaviorTurns=0;


    Game.battle.cpu.spin=100;
    Game.battle.cpu.balance=100;
    Game.battle.cpu.momentum=0;
    Game.battle.cpu.behavior="circling";
    Game.battle.cpu.behaviorTurns=0;
