...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=6.4`, `app.js?v=6.4`, `movement-engine.js?v=6.4`); hard-refresh after pulling.
...
- Live orbit is a Bit+RPM home ring (`homeOrbitRadius`), held by tangent speed `radius * omega`. Attack starts wide (outer bowl / X-Rail neighborhood) and slowly walks in as RPM dies: outer laps → mid-bowl → inner ring. It should not live on the rail all battle, and it should not collapse to a center pin at 40% RPM. Stamina stays a smaller ring than Attack at the same RPM, not a perfect center pin. After a clash, `impactMomentumState` owns the path so the Bey flies; the bowl slope then eases it back to the home ring. Do not yank to center, and do not accelerate free-space speed up to a separate cruise target.
