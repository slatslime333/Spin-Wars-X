...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=6.1`, `app.js?v=6.1`, `movement-engine.js?v=6.1`); hard-refresh after pulling.
...
- Non-attack Bits tighten toward center as RPM falls. Attack Bits stay wider at high RPM but sit inside the X-Rail (max preferred ~0.44), look tighter by ~75%, and are not out by the X-Rail at ~30%. After a clash, `impactMomentumState` owns the path briefly so the Bey flies, then orbit arcs back in.
