...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=6.6`, `app.js?v=6.6`, `movement-engine.js?v=6.6`); hard-refresh after pulling.
...
- Live orbit follows Bit type, then stats. A launch should spiral into a visible circle: Attack wider, Defense/Stamina smaller but still orbiting. Attack can reach the X-Rail from leftover width or a clash, but a Center launch should not immediately hug and ride it. Non-attack tightens faster as RPM drops. Kick/Taper sit between. Point/Level stay non-attack until their gimmick opens. After a clash, `impactMomentumState` owns the path; do not yank to a drawn ring or accelerate to a separate cruise target.
