...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=5.8`, `app.js?v=5.8`, `movement-engine.js?v=5.8`); hard-refresh after pulling.
...
- After a clash, `impactMomentumState` owns the path. Orbit steering must fade back in; it must not snap the Bey onto a ring the next frame.
