...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=7.5`, `app.js?v=7.5`, `movement-engine.js?v=7.5`); hard-refresh after pulling.
...
- Knockback is a bounce from actual closing speed. Do not invent extra shove from Attack/KB stats, mass, or a second energy formula. Light/glancing clashes stay planted. Fast head-on clashes can still fly (`impactSpeed` ~0.040+ and direct). `impactMomentumState` follows that shove; light hits must not lock orbit off. Speed-target damping must not eat impact velocity while `impactMomentumState` is high. Separating overlaps still must not stack.
...
- Live orbit follows Bit type, then stats. Home radius is a soft radial tendency, not a locked path. Free movement is a cartesian blend toward launch+RPM speed (`physicalSpeedTarget`); do not polar-rewrite heading or plant with bowl gravity. Attack hugs the X-Rail ring at high RPM, then walks in as spin drops. Defense/Stamina stay a little wider through high RPM, then drop into a tight center so two of them can actually meet. After a real smash, wall bounce, or X-Exit, `impactMomentumState` starves orbit steering so the Bey can fly. Ordinary clashes do not. X-Exit and free hits on the closer bounce toward center with leftover speed. While `railEngaged`, ride speed is owned by the X-Rail engine; do not cruise-damp it. Cruise accelerates up to `physicalSpeedTarget` and only bleeds excess; skip it while impact/exit is hot.
