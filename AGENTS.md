...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=7.1`, `app.js?v=7.1`, `movement-engine.js?v=7.1`); hard-refresh after pulling.
...
- Live orbit follows Bit type, then stats. A launch should spiral into a visible circle: Attack starts as wide as the X-Rail ring and stays wide through mid RPM, walking in only as spin really falls off; Defense/Stamina stay smaller with less magnet-to-center. A rail-width Attack orbit can graze and get about 1–2 early rides from leftover launch width, but circling on that ring must not auto-hook every lap — capture still needs a real CCW entry bite. After a clash, `impactMomentumState` owns a short cartesian knockback fly that keeps most of their carried speed — do not stall them to a stop and wind a new orbit, do not polar-correct the hit into a sway, and do not leave them ice-skating as a ragdoll. Knockback itself stays punchy but capped so they cannot be launched or chain-glitched across the stadium. While `railEngaged`, ride speed is owned by the X-Rail engine; do not cruise-damp it.
