...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=6.3`, `app.js?v=6.3`, `movement-engine.js?v=6.3`); hard-refresh after pulling.
...
- Live orbit is a Bit+RPM home ring (`homeOrbitRadius`), held by tangent speed `radius * omega`. Attack stays wider than stamina at the same RPM, but Attack at ~40% RPM sits in the inner bowl (~0.14–0.18), not on the X-Rail. After a clash, `impactMomentumState` owns the path briefly so the Bey flies, then it falls back to that ring in about 1–2s. Do not accelerate free-space speed up to a separate cruise target.
