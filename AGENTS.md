...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=5.9`, `app.js?v=5.9`, `movement-engine.js?v=5.9`); hard-refresh after pulling.
...
- Non-attack Bits tighten toward center as RPM falls; they can still be knocked wide, then settle back. Attack Bits stay wider and faster. After a clash, `impactMomentumState` owns the path, then orbit fades back in.
