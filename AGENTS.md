...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=8.1`, `app.js?v=8.1`, `movement-engine.js?v=8.1`); hard-refresh after pulling.
...
- Center launches stay in the middle of the bowl but sit slightly toward that player's stadium side so two Center picks cannot spawn on top of each other.
