...
- Serve the repo root over HTTP (`python3 -m http.server 8080`) and open `index.html`. Scripts are cache-busted (`xrail-engine.js?v=7.2`, `app.js?v=7.2`, `movement-engine.js?v=7.2`); hard-refresh after pulling.
...
- Live orbit follows Bit type, then stats. The home ring is a soft tendency, not a locked path — Beys should drift, rebound, and restabilize with carried speed instead of snapping onto a drawn circle. Attack hugs the X-Rail ring through most of a fight and only walks in once spin is really dying, so they ride the rail instead of clipping the X-Exit tip. Defense/Stamina stay a little wider through high RPM, then drop into a tight center so two of them can actually meet. After a clash, `impactMomentumState` owns cartesian knockback that keeps most of their speed; do not stall them and wind a new orbit. While `railEngaged`, ride speed is owned by the X-Rail engine; do not cruise-damp it.
